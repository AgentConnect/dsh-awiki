...
# Model Proxy Quick Recharge design QA
...
- `快速充值` opened with `账户与充值` selected, balance `0.965055 CNY`, model disabled, the exact disclosure, amount `1.00`, and the recharge action visible.
- Enable and disable completed in the Browser; status, action, and success feedback updated without navigation or layout shift.
...
The source combines AWiki advanced settings and model billing under one AWiki page. The new menu split, removed third tab, grouped enable action, and official-API/pricing disclosure are intentional requested differences. The shared account summary, tab hierarchy, recharge row, spacing rhythm, and desktop composition remain visually aligned.
