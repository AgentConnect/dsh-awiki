import { n as downloadedAttachment } from "./sdk-adapter-7OFOqOME.mjs";
...
function legacyDocument(seed, useLegacyDefault) {
	const selectedSlot = seed.legacyTenantSlot ?? (useLegacyDefault ? AWIKI_BUILTIN_TENANT_CONFIG.legacyDefaultSlot : void 0);
...
const DSH_AWIKI_VERSION = "0.3.7";
const DSH_AWIKI_MODEL_PROXY_VERSION = "0.1.3";
...
async function checkAwikiUpdatePolicy(options) {
	...
	const currentPluginVersion = options.currentPluginVersion ?? "0.3.7";
	const base = {
...
const DEFAULT_AWIKI_GUEST_URL = DEFAULT_AWIKI_SERVICE_URL;
...
function resolveConfig(ctx, config) {
	...
	const guestGatewayUrl = config.guestGatewayUrl === void 0 ? void 0 : serviceUrl("guestGatewayUrl", config.guestGatewayUrl, allowInsecureLoopbackForTesting);
	...
	return {
		...
		...guestGatewayUrl === void 0 ? {} : { guestGatewayUrl },
		...
		*/
		...
		async discoverTenantCapabilities(timeoutMs) {
			...
			try {
				...
				const guestGatewayBaseUrl = publishedServiceBaseUrl(services, "guest_gateway", this.resolved.allowInsecureLoopbackForTesting) ?? (this.resolved.guestGatewayUrl === void 0 ? void 0 : this.resolved.guestGatewayUrl);
				return publish({
					tenantId: tenant.tenantId,
					generation,
					online: true,
