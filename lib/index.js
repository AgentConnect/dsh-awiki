import { r as downloadedAttachment, t as AwikiSdkError } from "./sdk-adapter-CBm_C_ZW.mjs";
...
function identityApprovalReason(service, agent, input, action) {
	if (typeof input !== "object" || input === null || Array.isArray(input)) throw new TypeError("AWiki Agent identity approval requires parsed arguments.");
	const args = input;
	if (args.scope !== "preset" && args.scope !== "session") throw new TypeError("AWiki Agent identity approval requires a valid scope.");
	const targetAgentId = args.target_agent_id;
	if (targetAgentId !== void 0 && typeof targetAgentId !== "string") throw new TypeError("AWiki Agent identity approval requires a valid target Agent.");
	const route = service.agentIdentityApprovalRoute(requireAgent(agent), targetAgentId, args.scope);
	const routeLabel = JSON.stringify(route.key);
	const prefix = action === "create" ? "Create one permanent remote AWiki Agent DID and attach" : "Attach or replace an existing AWiki identity on";
	return route.scope === "preset" ? `${prefix} preset ${routeLabel}. This affects every session using that preset.` : `${prefix} session ${routeLabel}. This affects only that session.`;
}
