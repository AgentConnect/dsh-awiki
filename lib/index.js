...
function expectedRevision(payload) {
	if (!isRecord$3(payload) || !Number.isSafeInteger(payload.expectedRevision) || payload.expectedRevision < 0) return;
...
function view(provider) {
	...
	if (descriptor === void 0 || !isRecord$3(descriptor.value) || typeof descriptor.value.domain !== "string") return;
...
function createAwikiSettingsRpcHandler(getProvider, tenantManagement) {
	return async (endpoint, payload, signal) => {
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.describeTenants) {
			if (!isRecord$3(payload) || tenantManagement === void 0) return unavailable();
			try {
				return {
					ok: true,
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.createTenant) {
			if (!isRecord$3(payload) || typeof payload.displayName !== "string" || typeof payload.domain !== "string" || tenantManagement === void 0) return badRequest();
			try {
				return {
					ok: true,
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.renameTenant) {
			if (!isRecord$3(payload) || typeof payload.tenantId !== "string" || typeof payload.displayName !== "string" || tenantManagement === void 0) return badRequest();
			try {
				return {
					ok: true,
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.switchTenant) {
			if (!isRecord$3(payload) || typeof payload.tenantId !== "string" || tenantManagement === void 0) return badRequest();
			try {
				return {
					ok: true,
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.describe) {
			if (!isRecord$3(payload)) return badRequest();
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.setDomain) {
			if (!isRecord$3(payload) || typeof payload.domain !== "string") return badRequest();
			let domain;
			try {
...
function conversation(value) {
	if (!isRecord$2(value)) invalidState();
...
function hiddenPreference(value) {
	if (!isRecord$2(value) || !Number.isSafeInteger(value.hiddenAt) || value.hiddenAt < 0) invalidState();
	return {
...
function preferences(value, expectedOwnerDid) {
	if (!isRecord$2(value) || value.version !== STORE_VERSION || value.ownerDid !== String(expectedOwnerDid) || !Array.isArray(value.hiddenConversations) || value.hiddenConversations.length > MAX_HIDDEN_CONVERSATIONS) invalidState();
...
function parseState(value, identityScopeHash) {
	if (isRecord$1(value) && value.version === 1) return emptyState(identityScopeHash);
	if (isRecord$1(value) && value.version === STATE_VERSION && typeof value.identityScopeHash === "string" && value.identityScopeHash !== identityScopeHash) return emptyState(identityScopeHash);
	if (!isRecord$1(value) || value.version !== STATE_VERSION || value.identityScopeHash !== identityScopeHash || !isRecord$1(value.conversations)) throw new TypeError("awiki: listener state is invalid");
	...
	for (const [conversationId, raw] of entries) {
		...
		if (!isRecord$1(raw)) throw new TypeError("awiki: listener conversation state is invalid");
...
function decodeDocument(value) {
	if (!isRecord(value) || value.schemaVersion !== 1 || value.officialCatalogVersion !== 1 || !Number.isSafeInteger(value.generation) || value.generation < 0 || typeof value.activeTenantId !== "string" || !Array.isArray(value.tenants)) return void 0;
