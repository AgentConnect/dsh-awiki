...
const DEFAULT_AWIKI_DOMAIN = "awiki.me";
...
const AWIKI_OFFICIAL_CATALOG_VERSION = 1;
const AWIKI_CHINA_TENANT_ID = "official-china";
const AWIKI_GLOBAL_TENANT_ID = "official-global";
const OFFICIALS = [{
...
function decodeDocument(value) {
	if (!isRecord$1(value) || value.schemaVersion !== 1 || value.officialCatalogVersion !== 1 || !Number.isSafeInteger(value.generation) || value.generation < 0 || typeof value.activeTenantId !== "string" || !Array.isArray(value.tenants)) return void 0;
...
function sameOfficialEndpoint(tenant, domain) {
	const origin = `https://${domain}`;
	return tenant.didHost === domain || tenant.backendBaseUrl.replace(/\/$/u, "") === origin || tenant.endpoints.userServiceUrl.replace(/\/$/u, "") === origin;
}
...
async function checkAwikiUpdatePolicy(options) {
	...
	try {
		const response = await (options.fetcher ?? fetch)(new URL("/downloads/dsh-awiki/stable/manifest.json", origin), {
			method: "GET",
			headers: {
		...
		if (response.status === 404 && options.tenant.kind === "custom" && cached === void 0) return {
			...base,
			offline: false,
			usedCache: false,
...
function statusFromPolicy(base, policy, checkedAt, offline, usedCache) {
	...
	return {
		...
		modelProxyRestricted: base.currentModelProxyVersion !== void 0 && compareVersions(base.currentModelProxyVersion, modelProxy.min_supported_version) < 0,
		checkedAt
	};
}
...
function decodePolicy(value, origin) {
	...
	if (compareVersions(plugin.min_supported_version, plugin.recommended_version) > 0 || compareVersions(modelProxy.min_supported_version, modelProxy.recommended_version) > 0) throw new Error("minimum version exceeds recommended version");
	return {
		product: PRODUCT,
		channel: CHANNEL,
...
const DEFAULT_AWIKI_SERVICE_URL = "https://awiki.me";
/** @deprecated Legacy Guest origin; runtime capability binding has no production fallback. */
const DEFAULT_AWIKI_GUEST_URL = "https://awiki.info";
/** Default authoritative AWiki message-service DID. */
const DEFAULT_AWIKI_MESSAGE_SERVICE_DID = "did:wba:awiki.me";
...
function resolveConfig(ctx, config) {
	...
	const userServiceUrl = serviceUrl("userServiceUrl", config.userServiceUrl ?? "https://awiki.me", allowInsecureLoopbackForTesting);
	const messageServiceUrl = serviceUrl("messageServiceUrl", config.messageServiceUrl ?? "https://awiki.me", allowInsecureLoopbackForTesting);
	const mailServiceUrl = serviceUrl("mailServiceUrl", config.mailServiceUrl ?? userServiceUrl, allowInsecureLoopbackForTesting);
	const messageServicePublicUrl = serviceUrl("messageServicePublicUrl", config.messageServicePublicUrl ?? "https://awiki.me", allowInsecureLoopbackForTesting);
	...
	return {
		userServiceUrl,
		userServiceDomain: serviceDomain(config.userServiceDomain ?? "awiki.me"),
		messageServiceUrl,
		mailServiceUrl,
		messageServicePublicUrl,
		messageServiceDid: serviceDid(config.messageServiceDid ?? "did:wba:awiki.me"),
	...
	return class AwikiService extends _classSuper {
		...
		*/
		...
		ensureTenantRegistry() {
			...
			const seed = {
				domain,
				configured: domain !== "awiki.me" || this.resolved.userServiceUrl.replace(/\/$/u, "") !== configuredOrigin || this.resolved.messageServiceUrl.replace(/\/$/u, "") !== configuredOrigin,
		...
		async refreshUpdatePolicy() {
			...
			if (this.updatePolicyRequest === request && this.runtimeGeneration === generation && this.activeTenant?.tenantId === tenant.tenantId) this.updatePolicyStatus = status;
		...
		*/
		...
		async getSession() {
			...
			const identity = await this.run((client) => client.getIdentity(), {
				allowSignedOut: true,
				allowVersionRestricted: true
			});
		...
		*/
		clearLocalData(request) {
			...
			return this.mutateSession(async () => {
				const provider = this.provider;
				if (provider !== void 0) await this.stopProviderRuntime(provider);
				const result = await this.run((client) => client.clearLocalData(), {
					allowSignedOut: true,
					allowVersionRestricted: true
				});
				if (!result.ok) {
		...
		async run(operation, options = {}) {
			try {
				const generation = this.runtimeGeneration;
				if (options.allowVersionRestricted !== true && this.getUpdatePolicyStatus().restricted) return {
					ok: false,
