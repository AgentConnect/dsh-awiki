import { a as fencedMailRecoveryObservability, i as failedMailRecoveryObservability, o as successfulMailRecoveryObservability, r as downloadedAttachment, t as AwikiSdkError } from "./sdk-adapter-mYw3hEdF.mjs";
...
let AwikiService = (() => {
	...
	return class AwikiService extends _classSuper {
		...
		*/
		...
		openTenantProvider(tenant, factory) {
			const options = this.optionsForTenant(tenant);
			const provider = {
		...
		async switchTenant(tenantId) {
			...
			try {
				...
				if (this.provider === void 0) try {
					this.openTenantProvider(previousTenant, factory);
				} catch (rollbackError) {
					rollbackFailures.push(rollbackError);
				}
