import { n as downloadedAttachment } from "./sdk-adapter-DueSLoRb.mjs";
...
var AwikiTenantRegistry = class AwikiTenantRegistry {
	...
	static open(baseStateRoot, seed) {
		...
		const document = seed.configured || directoryHasData(baseStateRoot) ? legacyDocument(baseStateRoot, seed) : freshDocument();
...
const DSH_AWIKI_VERSION = "0.3.9";
const DSH_AWIKI_MODEL_PROXY_VERSION = "0.1.4";
...
async function checkAwikiUpdatePolicy(options) {
	...
	const currentPluginVersion = options.currentPluginVersion ?? "0.3.9";
	const base = {
