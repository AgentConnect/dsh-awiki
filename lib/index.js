import { a as fencedMailRecoveryObservability, i as failedMailRecoveryObservability, o as successfulMailRecoveryObservability, r as downloadedAttachment, s as isMessageTarget, t as AwikiSdkError } from "./sdk-adapter-DGcTiEZz.mjs";
...
//#endregion
//#region lib/types/integration-client.js
...
const OPERATIONS = {
	read: {
		method: "GET",
		path: "/guest/api/v1/management/integration"
	},
	create: {
		method: "POST",
		path: "/guest/api/v1/management/integration"
	...
	rotate: {
		method: "POST",
		path: "/guest/api/v1/management/integration/rotate-id"
	},
	close: {
		method: "POST",
		path: "/guest/api/v1/management/integration/close"
	},
	reopen: {
		method: "POST",
		path: "/guest/api/v1/management/integration/reopen"
	}
};
...
function view$1(raw) {
	...
	if (typeof owner !== "object" || owner === null || !Array.isArray(groups)) throw new TypeError("invalid Integration response");
	return {
		id: requiredString$1(value.id),
		publicId: nullableString(value.public_id),
		integrationUrl: nullableString(value.integration_url),
		owner: {
			tenantId: requiredString$1(owner.tenant_id),
			handle: requiredString$1(owner.handle),
			currentDid: requiredString$1(owner.current_did),
			displayName: requiredString$1(owner.display_name)
		},
		productName: requiredString$1(value.product_name),
		description: requiredString$1(value.description),
		contactEnabled: requiredBoolean(value.contact_enabled),
		contactDescription: requiredString$1(value.contact_description),
		groupTargets: groups.map((group) => {
			if (typeof group !== "object" || group === null) throw new TypeError("invalid Integration group");
			const target = group;
			return {
				id: requiredString$1(target.id),
				groupDid: requiredString$1(target.group_did),
				displayName: requiredString$1(target.display_name),
				avatarUrl: nullableString(target.avatar_url),
				description: requiredString$1(target.description),
...
function requiredEnum(value, allowed) {
	if (typeof value !== "string" || !allowed.includes(value)) throw new TypeError("invalid Integration response");
	return value;
}
function failure$1(status, raw) {
	const body = typeof raw === "object" && raw !== null ? raw : {};
	const nested = typeof body.error === "object" && body.error !== null ? body.error : body;
	const candidate = typeof nested.code === "string" ? nested.code : "";
	const code = status === 404 ? "not-found" : status === 409 ? "conflict" : status === 400 ? "invalid-request" : status === 401 ? "unauthorized" : status === 403 ? "forbidden" : status === 429 ? "rate-limited" : status >= 500 ? "unavailable" : [
		"not-found",
		"conflict",
		"invalid-request",
		"forbidden",
		...
		"rate-limited",
		"unavailable"
	].includes(candidate) ? candidate : "remote";
	return {
		code,
		message: integrationFailureMessage(code)
	};
}
...
async function readBounded$1(response) {
	...
	try {
		while (true) {
			...
			if (size > MAX_RESPONSE_BYTES$1) throw new TypeError("Integration response too large");
...
var AwikiIntegrationClient = class {
	origin;
	auth;
	...
	async execute(operation, body, idempotencyKey) {
		...
		const encoded = body === void 0 ? void 0 : JSON.stringify(body);
		if (encoded !== void 0 && Buffer.byteLength(encoded, "utf8") > MAX_REQUEST_BYTES) return {
			ok: false,
			error: {
				code: "invalid-request",
				message: integrationFailureMessage("invalid-request")
			}
		};
		const headers = new Headers({ accept: "application/json" });
		if (encoded !== void 0) headers.set("content-type", "application/json");
		...
		try {
			...
			const raw = await readBounded$1(response);
			if (!response.ok) return {
				ok: false,
				error: failure$1(response.status, raw)
			};
			return {
				ok: true,
				value: view$1(raw)
			...
			if (error instanceof AwikiExternalHttpAuthError && (error.code === "not-registered" || error.code === "signed-out")) return {
				ok: false,
				error: {
					code: "unauthorized",
					message: integrationFailureMessage("unauthorized")
				}
			};
			return {
				ok: false,
				error: {
					code: "network",
					message: integrationFailureMessage("network")
				}
			};
		} finally {
...
//#endregion
//#region lib/types/legacy-sent-mail-cache.js
...
async function clearLegacySentMailCache(stateRoot) {
	...
	try {
		...
		if (!host.isDirectory() || host.isSymbolicLink()) throw new TypeError("awiki: legacy sent-mail cache parent is invalid");
		const target = await lstat(directory);
		if (target.isDirectory() && !target.isSymbolicLink()) await rm(directory, {
			recursive: true,
			force: true
		});
		else await unlink(directory);
	} catch (error) {
		if (!isMissing$4(error)) throw error;
	}
}
//#endregion
//#region lib/types/mail-list-client.js
/** Host-only server-authoritative outbound Mail query. */
const MAIL_LIST_PATH = "/mail/rpc";
const MAIL_LIST_PAGE_SIZE = 100;
const MAIL_LIST_TIMEOUT_MS = 1e4;
...
function uint32(value) {
	if (!Number.isSafeInteger(value) || value < 0 || value > MAX_UINT32) invalidResponse();
	return value;
}
function requiredString(value, maxBytes) {
	if (typeof value !== "string" || value.length === 0 || value.trim() !== value || /[\u0000-\u001f\u007f-\u009f]/u.test(value) || Buffer.byteLength(value, "utf8") > maxBytes) invalidResponse();
	return value;
}
function addresses(value) {
	if (value === null || value === void 0 || value === "") return [];
	const raw = Array.isArray(value) ? value : typeof value === "string" ? value.split(/[;,]/u) : invalidResponse();
...
function timestamp(value) {
	...
	if (year === 0 || month < 1 || month > 12 || day < 1 || day > monthDays[month - 1] || hour > 23 || minute > 59 || second > 59) invalidResponse();
	if (zone !== void 0 && zone !== "Z") {
		const offsetHour = Number(zone.slice(1, 3));
		const offsetMinute = Number(zone.slice(4, 6));
		if (offsetHour > 23 || offsetMinute > 59) invalidResponse();
	}
	return zone === void 0 ? `${copied}Z` : copied;
}
function boundedText$1(value, maxBytes, fallback) {
...
function summary(raw) {
	...
	const subject = boundedText$1(value.subject, 1024, "(no subject)");
	return {
		id: requiredString(value.id, 2048),
		folder: "sent",
...
async function readBounded(response) {
	...
	const reader = response.body?.getReader();
	if (reader === void 0) invalidResponse();
	const chunks = [];
	let size = 0;
	try {
		while (true) {
...
function rpcFailure(code) {
	if (code === -32e3) throw new AwikiSdkError("not-registered");
	...
	async page(page) {
		...
		try {
			...
			if (envelope.error !== null && envelope.error !== void 0) {
				...
				rpcFailure(envelope.error.code);
			}
			if (typeof envelope.result !== "object" || envelope.result === null || Array.isArray(envelope.result)) invalidResponse();
...
function expectedRevision(payload) {
	if (!isRecord$4(payload) || !Number.isSafeInteger(payload.expectedRevision) || payload.expectedRevision < 0) return;
...
function view(provider) {
	...
	if (descriptor === void 0 || !isRecord$4(descriptor.value) || typeof descriptor.value.domain !== "string") return;
...
function publicTenantView(value) {
	return {
		...
		tenants: value.tenants.map((tenant) => {
			const slot = tenant.tenantId === "builtin-primary" ? "primary" : tenant.tenantId === "builtin-secondary" ? "secondary" : void 0;
			return {
				tenantId: tenant.tenantId,
				storageScopeId: tenant.storageScopeId,
				kind: tenant.kind,
				displayName: tenant.displayName,
				...slot === void 0 ? {} : { displayNames: AWIKI_BUILTIN_TENANT_CONFIG.tenants[slot].displayName },
...
function tenantRejected(error) {
	return {
		...
		error: {
			code: "settings-rejected",
			message: error instanceof Error && error.message.startsWith("awiki:") ? error.message : "The Host rejected the AWiki tenant change.",
			details: { ns: AWIKI_SETTINGS_NAMESPACE }
		}
	};
}
/** Build a handler whose provider lookup remains correct across Cordis reinjection. */
function createAwikiSettingsRpcHandler(getProvider, tenantManagement) {
	return async (endpoint, payload, signal) => {
		if (signal.aborted) return cancelled();
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.describeDesktopUpdate || endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.refreshDesktopUpdate) {
			if (!isRecord$4(payload)) return badRequest();
			try {
				return {
					ok: true,
					value: (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.refreshDesktopUpdate ? await tenantManagement?.refreshDesktopUpdate?.() : tenantManagement?.describeDesktopUpdate?.()) ?? null
				};
			} catch {
				return unavailable();
			}
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.describeTenants) {
			if (!isRecord$4(payload) || tenantManagement === void 0) return unavailable();
			try {
				return {
					ok: true,
					value: publicTenantView(tenantManagement.describe())
				};
			} catch (error) {
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.createTenant) {
			if (!isRecord$4(payload) || typeof payload.displayName !== "string" || typeof payload.domain !== "string" || tenantManagement === void 0) return badRequest();
			try {
				return {
					ok: true,
					value: publicTenantView(tenantManagement.create(payload.displayName, payload.domain))
				};
			} catch (error) {
				return tenantRejected(error);
			}
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.renameTenant) {
			if (!isRecord$4(payload) || typeof payload.tenantId !== "string" || typeof payload.displayName !== "string" || tenantManagement === void 0) return badRequest();
			try {
				return {
					ok: true,
					value: publicTenantView(tenantManagement.rename(payload.tenantId, payload.displayName))
				};
			} catch (error) {
				return tenantRejected(error);
			}
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.switchTenant) {
			if (!isRecord$4(payload) || typeof payload.tenantId !== "string" || tenantManagement === void 0) return badRequest();
			try {
				return {
					ok: true,
					value: publicTenantView(await tenantManagement.switch(payload.tenantId))
				};
			} catch (error) {
				return tenantRejected(error);
			}
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.archiveTenant) {
			...
			try {
				return {
					ok: true,
					value: publicTenantView(tenantManagement.archive(payload.tenantId))
				};
			} catch (error) {
				return tenantRejected(error);
			}
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.describeUpdatePolicy) {
			...
			if (!isRecord$4(payload) || describeUpdate === void 0) return unavailable();
			try {
				return {
					ok: true,
					value: describeUpdate.call(tenantManagement)
				};
			} catch (error) {
				return tenantRejected(error);
			}
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.refreshUpdatePolicy) {
			...
			if (!isRecord$4(payload) || refreshUpdate === void 0) return unavailable();
			try {
				return {
					ok: true,
					value: await refreshUpdate.call(tenantManagement)
				};
			} catch (error) {
				return tenantRejected(error);
			}
		...
		if (endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.setDomain) {
			if (!isRecord$4(payload) || typeof payload.domain !== "string") return badRequest();
			let domain;
			try {
...
function conversation(value) {
	if (!isRecord$3(value)) invalidState();
...
function hiddenPreference(value) {
	if (!isRecord$3(value) || !Number.isSafeInteger(value.hiddenAt) || value.hiddenAt < 0) invalidState();
	return {
...
function preferences(value, expectedOwnerDid) {
	if (!isRecord$3(value) || value.version !== STORE_VERSION || value.ownerDid !== String(expectedOwnerDid) || !Array.isArray(value.hiddenConversations) || value.hiddenConversations.length > MAX_HIDDEN_CONVERSATIONS) invalidState();
...
function parseState(value, identityScopeHash) {
	if (isRecord$2(value) && value.version === 1) return emptyState(identityScopeHash);
	if (isRecord$2(value) && value.version === STATE_VERSION && typeof value.identityScopeHash === "string" && value.identityScopeHash !== identityScopeHash) return emptyState(identityScopeHash);
	if (!isRecord$2(value) || value.version !== STATE_VERSION || value.identityScopeHash !== identityScopeHash || !isRecord$2(value.conversations)) throw new TypeError("awiki: listener state is invalid");
	...
	for (const [conversationId, raw] of entries) {
		...
		if (!isRecord$2(raw)) throw new TypeError("awiki: listener conversation state is invalid");
...
const SYNC_FAILURE_CODES = /* @__PURE__ */ new Set([
...
function syncFailureCode(error) {
	try {
		...
		return SYNC_FAILURE_CODES.has(value) ? value : void 0;
	} catch {
		return;
	}
}
...
var IdentityRealtimeSupervisor = class {
	...
	diagnostics() {
		return {
			...
			...this.lastFailureCode === void 0 ? {} : { lastFailureCode: this.lastFailureCode },
			...this.lastConnectedAtMs === void 0 ? {} : { lastConnectedAtMs: this.lastConnectedAtMs },
			...this.lastCommittedSyncCause === void 0 ? {} : { lastCommittedSyncCause: this.lastCommittedSyncCause },
			...this.lastSyncPagesFetched === void 0 ? {} : { lastSyncPagesFetched: this.lastSyncPagesFetched },
			...this.lastSyncMessagesHydrated === void 0 ? {} : { lastSyncMessagesHydrated: this.lastSyncMessagesHydrated },
			...this.lastSyncOlderHistoryExcluded === void 0 ? {} : { lastSyncOlderHistoryExcluded: this.lastSyncOlderHistoryExcluded }
		};
	}
	...
	async run(generation) {
		...
		while (this.current(generation)) {
			...
			try {
				...
				if (this.lastFailureCode !== "stop_failed") this.lastFailureCode = failedPhase === "initial_sync" || failedPhase === "reconnect_sync" ? safeSyncFailureCode ?? "sync_failed" : failedPhase === "starting" ? "start_failed" : "session_failed";
...
//#endregion
//#region lib/types/tenant-registry.js
/** Host-owned durable AWiki tenant catalog and storage-scope migration. */
const AWIKI_TENANT_REGISTRY_SCHEMA_VERSION = 1;
const AWIKI_OFFICIAL_CATALOG_VERSION = 2;
const AWIKI_PRIMARY_TENANT_ID = "builtin-primary";
const AWIKI_SECONDARY_TENANT_ID = "builtin-secondary";
/** Compatibility aliases for existing callers; the persisted IDs are generic slots. */
const AWIKI_CHINA_TENANT_ID = AWIKI_PRIMARY_TENANT_ID;
const AWIKI_GLOBAL_TENANT_ID = AWIKI_SECONDARY_TENANT_ID;
...
const PRE_REGISTRY_DEFAULT_ENDPOINTS = endpointsForOrigin(`https://${PRE_REGISTRY_DEFAULT_DOMAIN}`, PRE_REGISTRY_DEFAULT_DOMAIN);
...
function decodeEndpoints(value) {
	...
	if (userServiceUrl === void 0 || messageServiceUrl === void 0 || mailServiceUrl === void 0 || messageServicePublicUrl === void 0 || messageServiceDid === void 0) return void 0;
	return {
		userServiceUrl,
		messageServiceUrl,
		mailServiceUrl,
...
function decodeTenant(value) {
	...
	if (tenantId === void 0 || storageScopeId === void 0 || displayName === void 0 || backendBaseUrl === void 0 || didHost === void 0 || endpoints === void 0 || value.kind !== "built_in" && value.kind !== "custom" || value.lifecycle !== "active" && value.lifecycle !== "inactive" && value.lifecycle !== "archived" || value.storageLayout !== "scope-v1" && value.storageLayout !== "legacy-base" && value.storageLayout !== "domain-v1") return;
	return {
		tenantId,
		storageScopeId,
		kind: value.kind,
...
function decodeDocument(value) {
	if (!isRecord$1(value) || value.schemaVersion !== 1 || !Number.isSafeInteger(value.officialCatalogVersion) || value.officialCatalogVersion < 1 || value.officialCatalogVersion > 2 || !Number.isSafeInteger(value.generation) || value.generation < 0 || typeof value.activeTenantId !== "string" || !Array.isArray(value.tenants)) return void 0;
	const tenants = value.tenants.map(decodeTenant);
	if (tenants.some((tenant) => tenant === void 0)) return void 0;
	const complete = tenants;
	if (new Set(complete.map((tenant) => tenant.tenantId)).size !== complete.length || new Set(complete.map((tenant) => tenant.storageScopeId)).size !== complete.length || !complete.some((tenant) => tenant.tenantId === value.activeTenantId && tenant.lifecycle === "active") || complete.filter((tenant) => tenant.lifecycle === "active").length !== 1) return void 0;
	return {
		schemaVersion: 1,
		officialCatalogVersion: 2,
		generation: value.generation,
		activeTenantId: value.activeTenantId,
		tenants: complete
	};
...
function sameOfficialEndpoint(tenant, entry) {
	...
	return tenant.didHost === entry.didHost || tenant.backendBaseUrl.replace(/\/$/u, "") === origin || tenant.endpoints.userServiceUrl.replace(/\/$/u, "") === origin;
}
...
function reconcileOfficials(document) {
	...
	for (const entry of OFFICIALS) {
		...
		if (endpointIndex >= 0) {
			...
			if (activeTenantId === current.tenantId) activeTenantId = entry.tenantId;
		} else tenants.push(officialProfile(entry));
	}
	return activate({
		...document,
		officialCatalogVersion: 2,
...
var AwikiTenantRegistry = class AwikiTenantRegistry {
	baseStateRoot;
	filePath;
	backupPath;
	diagnosticPath;
	...
	static open(baseStateRoot, seed) {
		...
		if (existsSync(filePath)) try {
			const decoded = decodeDocument(JSON.parse(readFileSync(filePath, "utf8")));
			if (decoded === void 0) throw new Error("registry schema or invariants are invalid");
			...
			writeFileSync(`${filePath}.migration-error.txt`, diagnostic, {
				encoding: "utf8",
				mode: 384
			});
		...
		const document = seed.configured || hasLegacyData ? legacyDocument(seed, hasLegacyData && !seed.configured) : freshDocument();
	...
	active() {
		...
		if (tenant === void 0) throw new Error("awiki: active tenant is unavailable");
		return structuredClone(tenant);
	}
	find(tenantId) {
		const tenant = this.document.tenants.find((candidate) => candidate.tenantId === tenantId);
		return tenant === void 0 ? void 0 : structuredClone(tenant);
	}
	stateRoot(tenant) {
		if (tenant.storageLayout === "legacy-base") return this.baseStateRoot;
		if (tenant.storageLayout === "domain-v1") return join(dirname(this.baseStateRoot), "tenants", tenant.didHost, basename(this.baseStateRoot));
	...
	createCustom(displayName, rawDomain) {
		...
		if (this.document.tenants.some((tenant) => tenant.didHost === domain || tenant.backendBaseUrl === `https://${domain}`)) throw new Error("awiki: a tenant with this endpoint already exists");
		const tenant = {
			tenantId: randomUUID(),
		...
		this.document = {
			...this.document,
			tenants: [...this.document.tenants, tenant]
		};
		this.persist(true);
		return structuredClone(tenant);
	}
	renameCustom(tenantId, displayName) {
		...
		if (current === void 0) throw new Error("awiki: tenant does not exist");
	...
	commitActive(tenantId) {
		...
		if (target === void 0 || target.lifecycle === "archived") throw new Error("awiki: target tenant is unavailable");
...
function freshDocument() {
	...
	const activeTenantId = AWIKI_BUILTIN_TENANT_CONFIG.defaultSlot === "primary" ? AWIKI_PRIMARY_TENANT_ID : AWIKI_SECONDARY_TENANT_ID;
	return activate({
		schemaVersion: 1,
		officialCatalogVersion: 2,
		generation: 0,
...
function legacyDocument(seed, usePreRegistryDefault) {
	const selectedSlot = seed.legacyTenantSlot;
	const selectedEntry = selectedSlot === void 0 ? void 0 : OFFICIALS.find((candidate) => candidate.slot === selectedSlot);
	const historicalSeed = usePreRegistryDefault && selectedEntry === void 0 ? {
		...seed,
	...
	const endpoints = selectedEntry === void 0 ? {
...
const DSH_AWIKI_PACKAGE_VERSION = "0.3.16";
const DSH_AWIKI_MODEL_PROXY_PACKAGE_VERSION = "0.1.11";
//#endregion
//#region lib/types/update-policy.js
/** Tenant-scoped DSH AWiki plugin update policy and verified cache. */
const DSH_AWIKI_VERSION = DSH_AWIKI_PACKAGE_VERSION;
const DSH_AWIKI_MODEL_PROXY_VERSION = DSH_AWIKI_MODEL_PROXY_PACKAGE_VERSION;
const PRODUCT = "dsh-awiki";
const CHANNEL = "stable";
const MAX_POLICY_BYTES = 1048576;
/** Load only this tenant's verified cache before its business runtime starts. */
function readAwikiUpdatePolicyStatus(options) {
	...
	const base = {
		...
		...options.currentModelProxyVersion === void 0 ? {} : { currentModelProxyVersion: options.currentModelProxyVersion }
	};
...
async function checkAwikiUpdatePolicy(options) {
	...
	const timeout = AbortSignal.timeout(options.timeoutMs ?? 15e3);
	...
	try {
		...
		if (response.redirected || response.url !== "" && new URL(response.url).origin !== origin) throw new Error("update policy response crossed its tenant origin");
		...
		if (cached !== void 0 && policy.policy_revision < cached.policy.policy_revision) throw new Error("policy revision moved backwards");
...
function statusFromPolicy(base, policy, checkedAt, offline, usedCache, installedRuntime) {
	...
	return {
		...
		...modelProxy === void 0 || modelProxy.installable === false ? {} : { modelProxyTarget: publicTarget(modelProxy) },
		offline,
		usedCache,
		policyUnavailable: false,
		restricted: compareVersions(base.currentPluginVersion, plugin.min_supported_version) < 0,
		modelProxyRestricted: modelProxy !== void 0 && base.currentModelProxyVersion !== void 0 && compareVersions(base.currentModelProxyVersion, modelProxy.min_supported_version) < 0,
		checkedAt
	};
}
...
function publicTarget(value) {
	return {
		...
		...value.repository === void 0 ? {} : { repository: value.repository },
		...value.requires_plugin === void 0 ? {} : { requiresPlugin: value.requires_plugin }
	};
}
function decodePolicy(value, origin) {
	if (!isRecord(value) || value.product !== PRODUCT || value.channel !== CHANNEL || value.policy_origin !== origin || !Number.isSafeInteger(value.policy_revision) || value.policy_revision < 1 || typeof value.published_at !== "string" || Number.isNaN(Date.parse(value.published_at)) || typeof value.release_notes_url !== "string" || !isRecord(value.packages)) throw new Error("invalid update policy");
...
function decodeServerInfoPolicy(value, origin, minimumRevision = 0) {
	...
	if (!isRecord(releases) || releases.schema_version !== 1 || releases.channel !== CHANNEL || releases.policy_origin !== origin || !Number.isSafeInteger(releases.policy_revision) || releases.policy_revision < 1 || typeof releases.published_at !== "string" || Number.isNaN(Date.parse(releases.published_at)) || !isRecord(releases.products) || !isRecord(releases.products.dsh)) throw new Error("invalid client version policy");
	if (releases.policy_revision < minimumRevision) throw new Error("policy revision moved backwards");
	const product = releases.products.dsh;
	if (product.enabled === false) {
		if (product.compatibility === void 0 || product.compatibility === null) return void 0;
		...
		return decodePolicy({
			...
			packages: {
				...
				...product.compatibility.model_proxy == null ? {} : { model_proxy: component(product.compatibility.model_proxy, "@awiki/dsh-model-proxy") }
			}
		}, origin);
	}
	if (product.enabled !== true || typeof product.release_notes_url !== "string" || !isRecord(product.plugin)) throw new Error("invalid DSH client version policy");
	const plugin = decodeServerPackage(product.plugin, "@awiki/dsh-plugin");
	const modelProxy = isRecord(product.model_proxy) && product.model_proxy.enabled === true ? decodeServerPackage(product.model_proxy, "@awiki/dsh-model-proxy") : void 0;
	return decodePolicy({
		product: PRODUCT,
		channel: CHANNEL,
...
function decodeServerPackage(value, expectedName) {
	return decodePackage({
		...
		min_supported_version: value.minimum_supported_version,
		integrity: value.integrity,
		repository: value.repository === null ? void 0 : value.repository,
		requires_plugin: value.requires_plugin === null ? void 0 : value.requires_plugin
	}, expectedName);
}
function decodePackage(value, expectedName) {
	if (!isRecord(value) || value.name !== expectedName || typeof value.recommended_version !== "string" || typeof value.min_supported_version !== "string" || value.installable !== false && (typeof value.integrity !== "string" || !/^sha512-[A-Za-z0-9+/]+={0,2}$/u.test(value.integrity)) || value.repository !== void 0 && typeof value.repository !== "string" || value.requires_plugin !== void 0 && typeof value.requires_plugin !== "string") throw new Error("invalid update package target");
	...
	return {
		...
		...value.repository === void 0 ? {} : { repository: value.repository },
		...value.requires_plugin === void 0 ? {} : { requires_plugin: value.requires_plugin }
	};
}
...
function readCache(path, origin) {
	try {
		...
		if (!isRecord(value) || typeof value.checkedAt !== "string") return void 0;
		return {
			checkedAt: value.checkedAt,
			policy: decodePolicy(value.policy, origin)
		};
	} catch {
		return;
	}
...
const DEFAULT_AWIKI_SERVICE_URL = AWIKI_DEFAULT_BUILTIN_TENANT.backendOrigin;
/** Package-configured primary authoritative AWiki message-service DID. */
const DEFAULT_AWIKI_MESSAGE_SERVICE_DID = `did:wba:${AWIKI_DEFAULT_BUILTIN_TENANT.didHost}`;
...
function resolveConfig(ctx, config) {
	...
	if (legacyTenantSlot !== void 0 && legacyTenantSlot !== "primary" && legacyTenantSlot !== "secondary") throw new TypeError("awiki: legacyTenantSlot must be primary or secondary");
	return {
		userServiceUrl,
		userServiceDomain: serviceDomain(config.userServiceDomain ?? DEFAULT_AWIKI_DOMAIN),
		...legacyTenantSlot === void 0 ? {} : { legacyTenantSlot },
		messageServiceUrl,
		mailServiceUrl,
		messageServicePublicUrl,
		...
		...config.caBundle === void 0 ? {} : { caBundle: config.caBundle },
		stateRoot,
		attachmentMaxBytes,
...
let AwikiService = (() => {
	...
	let _getIdentityServices_decorators;
	let _updateIdentityServices_decorators;
	let _resumeIdentityServicesUpdate_decorators;
	let _startDeviceJoinVerification_decorators;
	let _approveDeviceJoin_decorators;
	let _retryDeviceManagement_decorators;
	...
	return class AwikiService extends _classSuper {
		static {
			...
			_getIdentityServices_decorators = [Remote];
			_updateIdentityServices_decorators = [Remote];
			_resumeIdentityServicesUpdate_decorators = [Remote];
			_startDeviceJoinVerification_decorators = [Remote];
			_approveDeviceJoin_decorators = [Remote];
			_retryDeviceManagement_decorators = [Remote];
			...
			__esDecorate(this, null, _getIntegration_decorators, {
				kind: "method",
				name: "getIntegration",
				static: false,
				private: false,
				access: {
					has: (obj) => "getIntegration" in obj,
					get: (obj) => obj.getIntegration
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _createIntegration_decorators, {
				kind: "method",
				name: "createIntegration",
				static: false,
				private: false,
				access: {
					has: (obj) => "createIntegration" in obj,
					get: (obj) => obj.createIntegration
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _updateIntegration_decorators, {
				kind: "method",
				name: "updateIntegration",
				static: false,
				private: false,
				access: {
					has: (obj) => "updateIntegration" in obj,
					get: (obj) => obj.updateIntegration
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _rotateIntegrationId_decorators, {
				kind: "method",
				name: "rotateIntegrationId",
				static: false,
				private: false,
				access: {
					has: (obj) => "rotateIntegrationId" in obj,
					get: (obj) => obj.rotateIntegrationId
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _closeIntegration_decorators, {
				kind: "method",
				name: "closeIntegration",
				static: false,
				private: false,
				access: {
					has: (obj) => "closeIntegration" in obj,
					get: (obj) => obj.closeIntegration
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _reopenIntegration_decorators, {
				kind: "method",
				name: "reopenIntegration",
				static: false,
				private: false,
				access: {
					has: (obj) => "reopenIntegration" in obj,
					get: (obj) => obj.reopenIntegration
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _retireDeviceIdentityForRejoin_decorators, {
				kind: "method",
				name: "retireDeviceIdentityForRejoin",
				static: false,
				private: false,
				access: {
					has: (obj) => "retireDeviceIdentityForRejoin" in obj,
					get: (obj) => obj.retireDeviceIdentityForRejoin
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _getIdentityAccessState_decorators, {
				kind: "method",
				name: "getIdentityAccessState",
				static: false,
				private: false,
				access: {
					has: (obj) => "getIdentityAccessState" in obj,
					get: (obj) => obj.getIdentityAccessState
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _getIdentityServices_decorators, {
				kind: "method",
				name: "getIdentityServices",
				static: false,
				private: false,
				access: {
					has: (obj) => "getIdentityServices" in obj,
					get: (obj) => obj.getIdentityServices
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _updateIdentityServices_decorators, {
				kind: "method",
				name: "updateIdentityServices",
				static: false,
				private: false,
				access: {
					has: (obj) => "updateIdentityServices" in obj,
					get: (obj) => obj.updateIdentityServices
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _resumeIdentityServicesUpdate_decorators, {
				kind: "method",
				name: "resumeIdentityServicesUpdate",
				static: false,
				private: false,
				access: {
					has: (obj) => "resumeIdentityServicesUpdate" in obj,
					get: (obj) => obj.resumeIdentityServicesUpdate
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _retryDeviceManagement_decorators, {
				kind: "method",
				name: "retryDeviceManagement",
				static: false,
				private: false,
				access: {
					has: (obj) => "retryDeviceManagement" in obj,
					get: (obj) => obj.retryDeviceManagement
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _prepareRootTransfer_decorators, {
				kind: "method",
				name: "prepareRootTransfer",
				static: false,
				private: false,
				access: {
					has: (obj) => "prepareRootTransfer" in obj,
					get: (obj) => obj.prepareRootTransfer
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _confirmRootTransfer_decorators, {
				kind: "method",
				name: "confirmRootTransfer",
				static: false,
				private: false,
				access: {
					has: (obj) => "confirmRootTransfer" in obj,
					get: (obj) => obj.confirmRootTransfer
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _enterRecoveredSession_decorators, {
				kind: "method",
				name: "enterRecoveredSession",
				static: false,
				private: false,
				access: {
					has: (obj) => "enterRecoveredSession" in obj,
					get: (obj) => obj.enterRecoveredSession
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _getDisplayProfiles_decorators, {
				kind: "method",
				name: "getDisplayProfiles",
				static: false,
				private: false,
				access: {
					has: (obj) => "getDisplayProfiles" in obj,
					get: (obj) => obj.getDisplayProfiles
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		tenantParticipants = /* @__PURE__ */ new Set();
		activeCapabilities;
		updatePolicyStatus;
		updatePolicyRequest;
		updatePolicyTask;
		...
		rootTransfers = /* @__PURE__ */ new Map();
		...
		*/
		constructor(ctx, config) {
			...
			ctx.inject(["desktopDistribution"], (desktopCtx) => {
				...
				if (typeof service !== "object" || service === null || !("getSnapshot" in service) || typeof service.getSnapshot !== "function" || !("check" in service) || typeof service.check !== "function") return;
				const candidate = service;
				if (decodeDesktopDistribution(candidate.getSnapshot()) === void 0) return;
				this.desktopDistribution = candidate;
				desktopCtx.effect(() => () => {
					if (this.desktopDistribution === candidate) this.desktopDistribution = void 0;
				});
			});
			ctx.effect(() => {
				const timer = setInterval(() => {
					this.refreshUpdatePolicy().catch(() => void 0);
				}, 216e5);
				timer.unref();
				return () => {
					clearInterval(timer);
		...
		*/
		registerClientFactory(factory) {
			if (this.clientFactory !== void 0 || this.provider !== void 0) throw new Error("awiki: a client provider is already registered");
			this.clientFactory = factory;
			try {
				this.openTenantProvider(this.ensureTenantRegistry().active(), factory);
			} catch (error) {
				this.clientFactory = void 0;
				throw error;
			}
			let active = true;
			return async () => {
				...
				this.clientFactory = void 0;
				const current = this.provider;
				if (current !== void 0) await this.disposeProvider(current);
			};
		}
		ensureTenantRegistry() {
			if (this.tenantRegistry !== void 0) return this.tenantRegistry;
			...
			const seed = {
				domain,
				configured: domain !== DEFAULT_AWIKI_DOMAIN || this.resolved.userServiceUrl.replace(/\/$/u, "") !== configuredOrigin || this.resolved.messageServiceUrl.replace(/\/$/u, "") !== configuredOrigin,
				...this.resolved.legacyTenantSlot === void 0 ? {} : { legacyTenantSlot: this.resolved.legacyTenantSlot },
				...
				messageServiceDid: this.resolved.messageServiceDid === `did:wba:${this.resolved.userServiceDomain}` ? `did:wba:${domain}` : this.resolved.messageServiceDid
			};
		...
		optionsForTenant(tenant) {
			...
			return {
				...
				allowedAttachmentOrigins: tenant.storageLayout === "legacy-base" ? this.resolved.allowedAttachmentOrigins : [messageServicePublicOrigin],
				...
				...this.resolved.caBundle === void 0 ? {} : { caBundle: this.resolved.caBundle },
				stateRoot: this.ensureTenantRegistry().stateRoot(tenant)
			};
		}
		ensureTenantOptions() {
			if (this.activeClientOptions !== void 0) return this.activeClientOptions;
		...
		bindTenantState(tenant, options) {
			this.updatePolicyRequest?.abort();
			this.updatePolicyStatus = void 0;
			this.updatePolicyTask = void 0;
		...
		refreshUpdatePolicy() {
			...
			if (this.updatePolicyTask !== void 0 && this.updatePolicyTaskKey === key) return this.updatePolicyTask;
			const task = this.performUpdatePolicyRefresh().finally(() => {
				if (this.updatePolicyTask === task) this.updatePolicyTask = void 0;
			});
		...
		async performUpdatePolicyRefresh() {
			...
			const status = await checkAwikiUpdatePolicy({
				...
				stateRoot: this.resolved.stateRoot,
				currentPluginVersion: DSH_AWIKI_VERSION,
				...currentModelProxyVersion === void 0 ? {} : { currentModelProxyVersion },
				allowInsecureLoopback: this.resolved.allowInsecureLoopbackForTesting,
				signal: request.signal
			});
			if (this.updatePolicyRequest === request && this.runtimeGeneration === generation && this.activeTenant?.tenantId === tenant.tenantId) {
				...
				} else if (wasRestricted && this.provider !== void 0 && !this.tenantSwitching) this.ensureProviderRuntime(this.provider);
			}
			return this.getUpdatePolicyStatus();
		}
		currentModelProxyVersion() {
			for (const participant of this.tenantParticipants) if (participant.component?.product === "dsh-awiki-model-proxy") return participant.component.version;
		}
		...
		registerTenantLifecycleParticipant(participant) {
			...
			if (participant.component !== void 0) this.updatePolicyStatus = void 0;
			this.refreshUpdatePolicy().catch(() => void 0);
			let active = true;
			return () => {
				if (!active) return;
		...
		async switchTenant(tenantId) {
			...
			if (factory === void 0 || previousProvider === void 0) throw new Error("awiki: client provider is unavailable");
			...
			try {
				...
				await Promise.all([this.discoverTenantCapabilities(3e3), this.refreshUpdatePolicy()]);
				...
				for (const participant of prepared.reverse()) try {
					await participant.rollbackSwitch?.(context);
				} catch (rollbackError) {
					rollbackFailures.push(rollbackError);
				}
				if (rollbackFailures.length > 0) throw new AggregateError([error, ...rollbackFailures], "awiki: tenant switch failed and rollback was incomplete");
				this.tenantSwitching = false;
				if (this.provider !== void 0) this.ensureProviderRuntime(this.provider);
		...
		*/
		async getConfig() {
			...
			const capabilities = await this.discoverTenantCapabilities(1e4);
			if (this.tenantSwitching || capabilities.generation !== this.runtimeGeneration || capabilities.tenantId !== this.activeTenant?.tenantId) return {
				ok: false,
			...
			return {
				...
				value: {
					...
					services: {
						modelProxy: { enabled: capabilities.modelProxyBaseUrl !== void 0 },
						guestGateway: { enabled: capabilities.guestGatewayBaseUrl !== void 0 }
					}
				}
			};
		}
		...
		async discoverTenantCapabilities(timeoutMs) {
			...
			const publish = (capabilities) => {
				if (this.runtimeGeneration === generation && this.activeTenant?.tenantId === tenant.tenantId) {
					...
					this.integrationClient = capabilities.guestGatewayBaseUrl === void 0 ? void 0 : new AwikiIntegrationClient(capabilities.guestGatewayBaseUrl, this.externalHttpAuth);
				}
				return capabilities;
			};
			const abort = new AbortController();
			...
			try {
				...
				if (typeof value !== "object" || value === null || value.schema_version !== 1) return publish(fallback);
				const methods = value.identity?.handle_recovery?.methods;
				const handleRecoveryPhoneEnabled = Array.isArray(methods) && methods.some((method) => typeof method === "object" && method !== null && method.id === "phone" && method.enabled === true && method.verification?.required === true && method.verification?.type === "sms_otp");
				...
				return publish({
					...
					...modelProxyBaseUrl === void 0 ? {} : { modelProxyBaseUrl },
					...guestGatewayBaseUrl === void 0 ? {} : { guestGatewayBaseUrl }
				});
			} catch {
		...
		*/
		...
		async inspectIdentityAccess(request) {
			...
			if (options === void 0) return {
				ok: false,
				error: normalizeFailure(new ProviderUnavailableError())
			};
			...
			if (request.inviteCode !== void 0 && (typeof request.inviteCode !== "string" || request.inviteCode.length > 512) || request.phone !== void 0 && (typeof request.phone !== "string" || request.phone.length > 64)) return {
				ok: false,
				error: failure("invalid-request")
			};
			...
			try {
				const response = await fetch(endpoint, {
					...
					body: JSON.stringify({
						...
						params: {
							...
							domain: options.userServiceDomain,
							check_invite: request.inviteCode !== void 0,
							...request.inviteCode === void 0 ? {} : { invite_code: request.inviteCode },
				...
				if (envelope?.jsonrpc !== "2.0" || envelope.id !== "registration-check" || envelope.error !== void 0 && envelope.error !== null || value?.full_handle !== target.fullHandle || ![
					"register",
					"existing",
					"unavailable"
				].includes(value.decision) || typeof value.invite_required !== "boolean" || ![
					"not_required",
					"required",
					"valid",
					"invalid"
				].includes(value.invite_status) || value.decision === "existing" && value.invite_required || !value.invite_required && value.invite_status !== "not_required" || value.invite_required && value.invite_status === "not_required") return {
					ok: false,
					error: failure("remote")
				};
				return {
					...
					value: {
						...
						inviteRequired: value.invite_required,
		...
		*/
		...
		async beginDeviceJoin() {
			...
			if (continuation.mode === "ordinary" && continuation.requiresUserPresence || continuation.mode === "handle-recovery-rebind" && !continuation.requiresUserPresence) return {
				ok: false,
			...
			const result = await this.run(async (client) => {
				...
				if (continuation.mode === "handle-recovery-rebind") {
					if (!client.trustedUserPresenceSupported) throw Object.assign(/* @__PURE__ */ new Error("trusted user presence unsupported"), {
						name: "AwikiSdkError",
						code: "forbidden"
					});
					userPresenceConfirmed = await client.confirmUserPresence("Rejoin your recovered AWiki identity on this device");
					if (!userPresenceConfirmed) throw Object.assign(/* @__PURE__ */ new Error("user presence denied"), {
						name: "AwikiSdkError",
						code: "forbidden"
					});
				}
		...
		getIdentityAccessState() {
			return this.run(async (client) => {
				...
				const capabilities = identity === null ? void 0 : await client.identityMethodCapabilities(identity.did);
				const domain = this.activeClientOptions?.userServiceDomain;
				const choice = this.pendingDeviceJoin;
				return {
					choice: joining || choice === void 0 ? null : {
						status: "join-required",
						fullHandle: choice.fullHandle,
						mode: choice.mode,
						requiresUserPresence: choice.requiresUserPresence,
						...choice.methodCapabilities === void 0 ? {} : { methodCapabilities: choice.methodCapabilities }
					},
					joining,
					creationMethods,
					pendingRegistrations,
					...capabilities === void 0 ? {} : { methodCapabilities: capabilities },
					recoveries: recoveries.filter((value) => domain !== void 0 && value.fullHandle.slice(value.fullHandle.indexOf(".") + 1) === domain)
				};
			}, { allowSignedOut: true });
		}
		...
		updateIdentityServices(request) {
			if (request === null || !Array.isArray(request.services)) return Promise.resolve({
				ok: false,
				error: failure("invalid-request")
			});
			return this.run(async (client) => {
				const did = await this.requireIdentityServicesManager(client);
				if (request.did !== did) throw Object.assign(/* @__PURE__ */ new Error("identity changed"), {
					name: "AwikiSdkError",
					code: "conflict"
				});
				if ((await client.getIdentityServices()).pending) throw Object.assign(/* @__PURE__ */ new Error("pending service update"), {
					name: "AwikiSdkError",
					code: "conflict"
				});
		...
		async requireIdentityServicesManager(client) {
			await this.requireDeviceManager(client);
			const identity = await client.getIdentity();
			if (identity === null || !(await client.identityMethodCapabilities(identity.did)).servicesUpdate) throw Object.assign(/* @__PURE__ */ new Error("service update unavailable"), {
				name: "AwikiSdkError",
				code: "forbidden"
			});
			return identity.did;
			...
			const result = await this.run(async (client) => {
				...
				try {
					...
					try {
						...
						if (adminJoinPhase(recovered) === "authorized") return recovered;
					} catch {}
					throw error;
				}
			});
		...
		async retryDeviceManagement(request) {
			...
			return this.run(async (client) => {
				...
				if (task === void 0 || task.phase !== "failed" || ["root_transfer.delivery_expired", "root_transfer.delivery_invalidated"].includes(task.failureCode ?? "")) throw Object.assign(/* @__PURE__ */ new Error("management retry unavailable"), {
					name: "AwikiSdkError",
					code: "forbidden"
				});
		...
		async prepareRootTransfer(request) {
			...
			return this.run(async (client) => {
				...
				if (preparation.recipient.did !== did || preparation.recipient.deviceId !== deviceId) throw Object.assign(/* @__PURE__ */ new Error("root transfer recipient mismatch"), {
					name: "AwikiSdkError",
					code: "remote"
				});
				for (const [reference, pending] of this.rootTransfers) if (pending.deviceId === deviceId) this.rootTransfers.delete(reference);
		...
		async confirmRootTransfer(request) {
			...
			return this.run(async (client) => {
				if (!await client.confirmUserPresence("Grant AWiki device management access")) throw Object.assign(/* @__PURE__ */ new Error("user presence denied"), {
					name: "AwikiSdkError",
					code: "forbidden"
				});
				if (await this.requireRootTransferRecipient(client, pending.deviceId) !== pending.did) throw Object.assign(/* @__PURE__ */ new Error("root transfer identity changed"), {
					name: "AwikiSdkError",
					code: "forbidden"
				});
				const sent = await client.confirmAndSendRootKeyTransfer(pending.authorizationHandle);
				if (sent.recipientDeviceId !== pending.deviceId) throw Object.assign(/* @__PURE__ */ new Error("root transfer receipt mismatch"), {
					name: "AwikiSdkError",
					code: "remote"
				});
				return {
		...
		*/
		...
		async enterRecoveredSession(request) {
			...
			if (!result.ok) return result;
			if (result.value.phase !== "applied" || !result.value.allowedActions?.includes("activate_identity")) return {
				ok: false,
				error: failure("conflict")
			};
		...
		*/
		...
		getDisplayProfiles(peers) {
			if (!Array.isArray(peers) || peers.length > 100 || peers.some((peer) => typeof peer !== "string" || !peer.startsWith("did:"))) return Promise.resolve({
				ok: false,
				error: failure("invalid-request")
			});
		...
		*/
		clearLocalData(request) {
			...
			return this.mutateSession(async () => {
				...
				if (!result.ok && result.error.code === "remote") result = await this.run((client) => client.clearLocalData(), { allowSignedOut: true });
				if (!result.ok) {
					if (provider !== void 0 && this.provider === provider) this.ensureProviderRuntime(provider);
					return result;
				}
		...
		async applyRecoveredSession(progress, requestGeneration) {
			if (progress.phase !== "applied") return {
				ok: true,
				value: progress
			};
			return this.mutateSession(async () => {
				...
				if (this.sessionRevision !== requestGeneration) return {
					ok: false,
					error: {
						...failure("remote"),
				...
				try {
					...
					if (this.provider !== provider || this.sessionRevision !== requestGeneration) return {
						ok: false,
						error: {
							...failure("remote"),
					...
					if (!currentPrincipalMatchesRecovery) return {
						ok: false,
						error: {
							...failure("remote"),
		...
		async selectDeviceJoinSession(client) {
			const resumable = (await client.listLocalDeviceJoinSessions()).filter((value) => value.side === "new_device" && RESUMABLE_JOIN_PHASES.has(value.localPhase));
		...
		async requireRootTransferRecipient(client, deviceId) {
			...
			await client.syncDeviceManagement();
			const identity = await client.getIdentity();
			if (identity === null) throw Object.assign(/* @__PURE__ */ new Error("identity required"), {
				name: "AwikiSdkError",
				code: "not-registered"
			});
			if (!(await client.identityMethodCapabilities(identity.did)).rootTransfer) throw Object.assign(/* @__PURE__ */ new Error("root transfer unavailable"), {
				name: "AwikiSdkError",
				code: "forbidden"
			});
			const target = (await client.getDeviceRegistry()).find((device) => device.deviceId === deviceId);
			if (target === void 0 || target.isCurrent || target.status !== "active" || target.role !== "member" || target.managementReady) throw Object.assign(/* @__PURE__ */ new Error("root transfer recipient unavailable"), {
				name: "AwikiSdkError",
				code: "forbidden"
			});
			return identity.did;
		...
		async deviceManagementSnapshot(client) {
			...
			const capabilities = currentIdentity === null ? void 0 : await client.identityMethodCapabilities(currentIdentity.did);
			...
			const identity = await client.getIdentity();
			const joinedAtByDeviceId = /* @__PURE__ */ new Map();
			for (const request of requests) {
				const previous = joinedAtByDeviceId.get(request.protocolDeviceId);
				if (previous === void 0 || request.issuedAt > previous) joinedAtByDeviceId.set(request.protocolDeviceId, request.issuedAt);
			}
			const initialDeviceJoinedAt = identity === null ? void 0 : new Date(identity.registeredAt).toISOString();
			return {
				canManage: true,
				rootTransferSupported,
				...capabilities === void 0 ? {} : { methodCapabilities: capabilities },
				...
				devices: devices.filter((device) => device.status === "active").map((device) => {
					...
					const phase = task?.phase === "failed" ? "failed" : task?.phase === "management_registered" ? "complete" : task?.phase === "waiting_for_recipient" ? "waiting" : "pending";
					return {
						...this.publicDevice(device, joinedAtByDeviceId.get(device.deviceId) ?? (device.isCurrent ? initialDeviceJoinedAt : void 0)),
						...task === void 0 ? {} : { provisioning: {
							phase,
							attempts: task.attempts,
							...["root_transfer.delivery_expired", "root_transfer.delivery_invalidated"].includes(task.failureCode ?? "") ? { requiresRejoin: true } : {}
						} }
					};
				}),
		...
		mutateSession(operation) {
			const generation = this.runtimeGeneration;
			if (this.tenantSwitching) return Promise.resolve({
				ok: false,
				error: failure("conflict")
			});
			const invoke = () => this.tenantSwitching || generation !== this.runtimeGeneration ? Promise.resolve({
				ok: false,
				error: failure("conflict")
			}) : operation();
