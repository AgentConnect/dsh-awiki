import { a as fencedMailRecoveryObservability, i as failedMailRecoveryObservability, o as successfulMailRecoveryObservability, r as downloadedAttachment, t as AwikiSdkError } from "./sdk-adapter-DOCdg24r.mjs";
...
var AwikiSessionStore = class {
	...
	async recoveryOperationId() {
		if (!await this.hasPrivateHostDirectory()) return null;
		try {
			const metadata = await lstat(this.recoveryOperationPath);
			if (!metadata.isFile() || metadata.isSymbolicLink() || metadata.size > 256) throw new TypeError("awiki: recovery operation marker is invalid");
			const marker = await readFile(this.recoveryOperationPath, "utf8");
			if (!marker.endsWith("\n") || marker.indexOf("\n") !== marker.length - 1) throw new TypeError("awiki: recovery operation marker is invalid");
			const operationId = marker.slice(22, -1);
			if (!marker.startsWith(RECOVERY_OPERATION_PREFIX) || !RECOVERY_OPERATION_PATTERN.test(operationId)) throw new TypeError("awiki: recovery operation marker is invalid");
			return operationId;
		} catch (error) {
			if (isMissing$3(error)) return null;
			throw error;
		}
	}
	...
	async setRecoveryOperationId(operationId) {
		if (operationId === null) {
			if (!await this.hasPrivateHostDirectory()) return;
			try {
				const metadata = await lstat(this.recoveryOperationPath);
				if (!metadata.isFile() || metadata.isSymbolicLink()) throw new TypeError("awiki: recovery operation marker is invalid");
		...
		if (!RECOVERY_OPERATION_PATTERN.test(operationId)) throw new TypeError("awiki: recovery operation id is invalid");
		await mkdir(this.hostDirectory, {
			recursive: true,
			mode: 448
		});
		...
		try {
			...
			await chmod(this.recoveryOperationPath, 384);
		} finally {
