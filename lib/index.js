		...
		*/
		...
		async retryDeviceManagement(request) {
			...
			return this.run(async (client) => {
				...
				if (task === void 0 || task.phase !== "failed" || task.failureCode === "root_transfer.delivery_expired") throw Object.assign(/* @__PURE__ */ new Error("management retry unavailable"), {
					name: "AwikiSdkError",
					code: "forbidden"
				});
				...
				devices: devices.filter((device) => device.status === "active").map((device) => {
					...
					return {
						...
						...task === void 0 ? {} : { provisioning: {
							...
							...task.failureCode === "root_transfer.delivery_expired" ? { requiresRejoin: true } : {}
						} }
					};
				}),
