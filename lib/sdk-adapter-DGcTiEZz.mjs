...
function methodCapabilities(value) {
	return {
		method: value.method,
		handleRecovery: boolean(value.handleRecovery),
		rootImport: boolean(value.rootImport),
		rootTransfer: boolean(value.rootTransfer),
		servicesUpdate: boolean(value.servicesUpdate)
	};
}
function publicDocumentService(raw) {
	...
	if (value.serviceDid !== void 0 && typeof value.serviceDid !== "string") fail("invalid-request");
	for (const field of ["profiles", "securityProfiles"]) if (value[field] !== void 0 && (!Array.isArray(value[field]) || !value[field].every((item) => typeof item === "string"))) fail("invalid-request");
	return {
		id: value.id,
		type: value.type,
		serviceEndpoint: value.serviceEndpoint,
		...value.serviceDid === void 0 ? {} : { serviceDid: value.serviceDid },
		...value.profiles === void 0 ? {} : { profiles: [...value.profiles] },
		...value.securityProfiles === void 0 ? {} : { securityProfiles: [...value.securityProfiles] }
	};
}
...
var RustSdkAdapter = class {
	...
	client;
	approvalManagement = /* @__PURE__ */ new Map();
	...
	*/
	...
	updateIdentityServices(request) {
		return this.run(async (client) => {
			await client.updateIdentityServices(request.services.map(publicDocumentService));
		});
	}
	...
	prepareDeviceJoinApproval(joinSessionId) {
		return this.run(async (client) => {
			...
			const value = await client.prepareDeviceJoinApproval({
				joinSessionId,
				sasConfirmed: true
			});
	...
	retryDeviceJoinManagement(joinSessionId) {
	...
	confirmDeviceJoinApproval(approvalHandle) {
		return this.run(async (client) => {
			...
			const result = this.approvalManagement.get(approvalHandle) === false ? await client.confirmDeviceJoinApproval(request) : await client.confirmDeviceJoinWithManagement(request);
