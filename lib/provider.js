import { t as TypeScriptSdkAdapter } from "./sdk-adapter-DnNw87C6.mjs";
...
function classifyRemoteError(input) {
	...
	if (serviceCode === "identity.registration_verification_invalid") return "invalid-otp";
	if (serviceCode === "identity.registration_verification_unavailable") return "challenge-expired";
	...
	if (serviceCode === "anp.idempotency_conflict" || serviceCode === "idempotency_conflict") return "conflict";
	...
	if (combined.includes("handle_unavailable") || combined.includes("handle_exists") || combined.includes("handle already") || message.includes("handle") && message.includes("已被占用")) return "handle-unavailable";
	if (combined.includes("already_registered") || combined.includes("did already")) return "already-registered";
	if (input.rpcCode === 1409) return "conflict";
...
function publicMessage(code) {
	switch (code) {
		...
		case "challenge-expired": return "AWiki registration challenge is no longer available";
...
var AwikiIdentityRuntime = class {
	...
	async sendRegistrationOtp(request) {
		...
		await this.options.store.mutate((state) => {
			...
			if (pending && (pending.handle !== registrationOtp.handle || pending.phone !== registrationOtp.phone)) delete state.pendingRegistration;
	...
	async clear() {
		...
		const operation = this.mutationTail.then(async () => {
			try {
				...
				if (!isMissingFile(error)) throw new AwikiImError("remote", "AWiki identity state cannot be cleared", void 0, { cause: error });
			}
			this.state = emptyState();
		});
		this.mutationTail = operation.catch(() => void 0);
...
var DefaultAwikiImClient = class {
	...
	run(operation) {
		if (this.disposed) return Promise.reject(new AwikiImError("remote", "AWiki IM client has been disposed"));
		if (this.clearing !== void 0) return Promise.reject(new AwikiImError("conflict", "AWiki local data is being cleared"));
		const pending = (async () => {
			try {
				await this.ready;
				return await operation();
	...
	async disposeOnce() {
		...
		await Promise.allSettled([...this.inFlight]);
		if (clearing !== void 0) await Promise.allSettled([clearing]);
		await ready;
	}
};
