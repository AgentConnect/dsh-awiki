...
function parseState(value, identityScopeHash) {
    if (isRecord(value) && value.version === 1)
        return emptyState(identityScopeHash);
    if (isRecord(value)
    ...
    if (entries.length > MAX_CONVERSATIONS)
        throw new TypeError('awiki: listener state is too large');
    const conversations = {};
...
export class AwikiListenerStateStore {
    ...
    async load() {
        if (!(await this.hasPrivateHostDirectory()))
            return emptyState(this.identityScopeHash);
        try {
...
//# sourceMappingURL=listener-state.js.map