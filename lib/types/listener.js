...
export class AwikiAgentListener {
    ...
    start() {
        ...
        this.lifecycle = this.run(generation).then(() => { this.finishTermination({ kind: 'stopped' }); }, (error) => {
    ...
    async run(lifecycleGeneration) {
        ...
        try {
            while (this.currentLifecycle(lifecycleGeneration)) {
                ...
                if (!this.currentLifecycle(lifecycleGeneration)
                    || this.activeRealtime?.generation !== active.generation)
                    return;
                if (event === null) {
                    this.activeRealtime = undefined;
                    this.streamGeneration += 1;
            ...
            if (this.activeRealtime?.generation === active.generation)
                this.activeRealtime = undefined;
            await active.session.stop().catch(() => undefined);
        }
    }
