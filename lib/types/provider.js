...
import { syncAnpIdentityHandle } from "./identity-handle.js";
...
async function waitForIdentityProvider(identity) {
    ...
    while (!providerIsReady(await identity.health())) {
        ...
        if (remaining <= 0) {
            throw new Error(`awiki: ANP Identity provider did not become ready within ${IDENTITY_PROVIDER_READY_TIMEOUT_MS}ms`);
        }
