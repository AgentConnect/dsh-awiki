...
import { AwikiExternalHttpAuthError, } from "./external-http-auth.js";
...
function addresses(value) {
    ...
    return raw.map(candidate => {
        ...
        if (!address.includes('@') || /\s/u.test(address))
            invalidResponse();
        return address;
    });
}
...
function boundedText(value, maxBytes, fallback) {
...
function summary(raw) {
    if (typeof raw !== 'object' || raw === null || Array.isArray(raw))
        invalidResponse();
    const value = raw;
    if (value.direction !== 'outbound' || typeof value.has_attachments !== 'boolean')
        invalidResponse();
    const subject = boundedText(value.subject, 1_024, '(no subject)');
    return {
        id: requiredString(value.id, 2_048),
        folder: 'sent',
...
function rpcFailure(code) {
    ...
    if (code === -32004 || code === -32600 || code === -32602)
    ...
    async page(page) {
        ...
        try {
            ...
            if (envelope.error !== null && envelope.error !== undefined) {
                if (typeof envelope.error !== 'object' || Array.isArray(envelope.error))
                    invalidResponse();
            ...
            if (!Array.isArray(result.items) || result.items.length > MAIL_LIST_PAGE_SIZE)
                invalidResponse();
            ...
            if (error instanceof AwikiExternalHttpAuthError) {
                if (error.code === 'not-registered')
                    throw new AwikiSdkError('not-registered');
                if (error.code === 'signed-out')
                    throw new AwikiSdkError('signed-out');
            }
            throw new AwikiSdkError('network');
        }
        finally {
...
//# sourceMappingURL=mail-list-client.js.map