...
import { registrationHandleLocalPart } from "./registration-policy.js";
import { isMessageTarget } from "./message-target.js";
...
import { AwikiMailAttachmentClient, MAIL_HTTP_UPLOAD_MAX_BYTES } from "./mail-attachment-client.js";
...
import { MAIL_ATTACHMENT_SERVICE_MAX_BYTES, MAIL_ATTACHMENT_SERVICE_MAX_COUNT, MAIL_ATTACHMENT_SERVICE_TOTAL_MAX_BYTES, mailAttachmentDownloadRequest, mailInboxRequest, mailMarkReadRequest, mailReadRequest, mailSendRequest, } from "./mail.js";
...
function normalizeSendTextRequest(request) {
    if (typeof request?.text !== 'string' || typeof request.idempotencyKey !== 'string'
        || !isMessageTarget(request.target))
        return undefined;
...
let AwikiService = (() => {
    ...
    return class AwikiService extends _classSuper {
        static {
            ...
            __esDecorate(this, null, _getIdentityServices_decorators, { kind: "method", name: "getIdentityServices", static: false, private: false, access: { has: obj => "getIdentityServices" in obj, get: obj => obj.getIdentityServices }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _updateIdentityServices_decorators, { kind: "method", name: "updateIdentityServices", static: false, private: false, access: { has: obj => "updateIdentityServices" in obj, get: obj => obj.updateIdentityServices }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _resumeIdentityServicesUpdate_decorators, { kind: "method", name: "resumeIdentityServicesUpdate", static: false, private: false, access: { has: obj => "resumeIdentityServicesUpdate" in obj, get: obj => obj.resumeIdentityServicesUpdate }, metadata: _metadata }, null, _instanceExtraInitializers);
            ...
            __esDecorate(this, null, _retryDeviceManagement_decorators, { kind: "method", name: "retryDeviceManagement", static: false, private: false, access: { has: obj => "retryDeviceManagement" in obj, get: obj => obj.retryDeviceManagement }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        getDesktopUpdate() {
            ...
            if (view === undefined || view.schemaVersion === 1)
                return view;
            const tenant = this.activeTenant ?? this.ensureTenantRegistry().active();
        ...
        updateIdentityServices(request) {
            if (request === null || !Array.isArray(request.services))
                return Promise.resolve({ ok: false, error: failure('invalid-request') });
            return this.run(async (client) => {
                const did = await this.requireIdentityServicesManager(client);
                if (request.did !== did)
                    throw Object.assign(new Error('identity changed'), { name: 'AwikiSdkError', code: 'conflict' });
        ...
        async sendAttachment(request) {
            if (!isMessageTarget(request?.target))
                return { ok: false, error: failure('invalid-request') };
        ...
        async sendMail(request) {
            ...
            return this.run(client => normalized.attachments.length === 0
                ? client.sendMail({ to: normalized.to, cc: normalized.cc, subject: normalized.subject, bodyText: normalized.bodyText })
        ...
        async deviceManagementSnapshot(client) {
            ...
            return {
                ...
                devices: devices
                    ...
                    const phase = task?.phase === 'failed' ? 'failed'
                        : task?.phase === 'management_registered' ? 'complete'
                            : task?.phase === 'waiting_for_recipient' ? 'waiting' : 'pending';
                    return {
        ...
        async run(operation, options = {}) {
            try {
                ...
                if (provider === undefined)
                    throw new ProviderUnavailableError();
                const owner = options.bindMailOwner === true ? await provider.client.getIdentity() : undefined;
                const assertMailSession = () => {
                    if (options.bindMailOwner !== true)
                        return;
                    if (this.runtimeGeneration !== generation || this.provider !== provider || this.tenantSwitching) {
                        throw new AwikiSdkError('conflict');
                    }
                    if (this.sessionRevision !== revision || this.signedOut === true)
                        throw new ProviderUnavailableError();
                    if (owner == null)
                        throw new AwikiSdkError('not-registered');
                    if (this.activeIdentityDid !== undefined && this.activeIdentityDid !== owner.did)
                        throw new ProviderUnavailableError();
                };
                assertMailSession();
