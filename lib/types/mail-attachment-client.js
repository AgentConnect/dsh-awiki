...
import { AWIKI_EXTERNAL_HTTP_MAX_BODY_BYTES, AwikiExternalHttpAuthError } from "./external-http-auth.js";
import { mailAttachmentContentType, mailAttachmentFileName } from "./mail.js";
import { standardBase64Syntax } from "./base64.js";
import { AwikiSdkError } from "./sdk-adapter.js";
...
function invalid() { throw new AwikiSdkError('remote'); }
...
function text(value, max) {
    if (typeof value !== 'string' || value.length === 0 || value.trim() !== value || /[\p{C}]/u.test(value) || Buffer.byteLength(value) > max)
        invalid();
    return value;
}
...
function serviceFailure(code) {
    ...
    if (code === 403 || code === 1403)
        throw new AwikiSdkError('forbidden');
    if (code === 404 || code === 1404)
        throw new AwikiSdkError('not-found');
    if (code === 413 || code === 1413)
        throw new AwikiSdkError('attachment-too-large');
    if (code === 429 || code === 1429)
        throw new AwikiSdkError('rate-limited');
    invalid();
}
export class AwikiMailAttachmentClient {
    ...
    async send(request) {
        ...
        if (result.accepted !== true || result.status !== 'sent'
            || (result.ok !== undefined && result.ok !== true) || (result.success !== undefined && result.success !== true))
            invalid();
        const messageId = result.message_id ?? result.messageId ?? result.id;
        const warnings = result.warnings ?? [];
        if (!Array.isArray(warnings) || warnings.length > 100)
            invalid();
        return {
            accepted: true,
            ...(messageId === undefined || messageId === null ? {} : { messageId: text(messageId, 2048) }),
    ...
    async download(request, maxBytes) {
        ...
        if (integer(result.index ?? result.attachment_index ?? result.attachmentIndex) !== request.attachmentIndex)
            invalid();
        ...
        if (typeof content !== 'string' || content.length !== Math.ceil(sizeBytes / 3) * 4 || !standardBase64Syntax(content))
            invalid();
        const bytes = Buffer.from(content, 'base64');
        if (bytes.byteLength !== sizeBytes || bytes.toString('base64') !== content)
            invalid();
    ...
    async rpc(method, params, maxResponseBytes, allowAuthRetry) {
        ...
        if (Buffer.byteLength(body) > AWIKI_EXTERNAL_HTTP_MAX_BODY_BYTES)
            throw new AwikiSdkError('attachment-too-large');
        ...
        try {
            ...
            if (error instanceof AwikiExternalHttpAuthError) {
                if (error.code === 'not-registered' || error.code === 'signed-out')
                    throw new AwikiSdkError(error.code);
...
//# sourceMappingURL=mail-attachment-client.js.map