                    ...
                    if (selected.provider === PROVIDER) {
                        await ctx.agentDefaultModel.saveSelection({
                            ...
                            ...stored.previousReasoningEffort === undefined
                                ? {}
                                : { reasoningEffort: stored.previousReasoningEffort },
                        });
                    }
                }
...
async function authenticatedJson(config, token, path, init) {
    ...
    if (!response.ok)
        throw await modelProxyError(response, `AWiki-hosted model service returned HTTP ${response.status}`);
...
function badRequest() {
    return {
        ok: false,
        error: { code: 'bad-request', message: 'The AWiki-hosted model request is invalid.', details: { issues: [] } },
    };
}
