...
export const AWIKI_SETTINGS_RPC_CHANNEL = '/api';
...
export function decodeAwikiUpdatePolicyRpcView(value) {
    ...
    if (value.policyRevision !== undefined
        ...
        return undefined;
    if (value.checkState !== undefined && !['unchecked', 'ready', 'unavailable', 'failed'].includes(String(value.checkState)))
        return undefined;
    if (value.updateAvailable !== undefined && typeof value.updateAvailable !== 'boolean')
        return undefined;
    if (value.upgradeBlockedReason !== undefined && !['installation-unverified', 'host-incompatible', 'identity-incompatible'].includes(String(value.upgradeBlockedReason)))
        return undefined;
    return value;
}
