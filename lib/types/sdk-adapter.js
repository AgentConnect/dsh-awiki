...
export class RustSdkAdapter {
    ...
    listenerSyncNow(reason) {
        return this.run(async (client) => {
            ...
            throw new AwikiSdkError(result.status === 'auth_revoked' ? 'identity-recovery-required' : 'network', realtimeSyncFailureCode(result.status, result.warnings, syncResultErrorCode(result)));
        });
    }
