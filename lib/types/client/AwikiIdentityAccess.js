...
export function AwikiIdentityAccess(props) {
    ...
    useEffect(() => {
        if (!registrationOtpSent && pendingRegistration === null && creationMethods.length > 0 && !creationMethods.includes(didMethod)) {
            setDidMethod(creationMethods.includes('wba') ? 'wba' : creationMethods[0]);
        }
    }, [props.access?.creationMethods, registrationOtpSent, pendingRegistration, didMethod]);
    useEffect(() => {
    ...
    const requestRegistrationOtp = async () => {
        ...
        try {
            ...
            if (checked.value.status === 'unavailable')
                return setError('此 Handle 暂不可注册，请更换后重试。');
            if (checked.value.status !== 'existing' && checked.value.inviteRequired && checked.value.inviteStatus !== 'valid') {
                ...
                setNotice(null);
                if (checked.value.inviteStatus === 'invalid')
                    setError('邀请码无效或不适用于当前账号，请检查后重试。');
                return;
            }
            if (checked.value.status !== 'existing' && !canRegister)
                return setError('当前服务暂不接受所选方法的新注册。');
            const admissionInvite = checked.value.status === 'existing' ? '' : inviteCode.trim();
            if (checked.value.status === 'existing') {
                setInviteCode('');
                setShortHandleInviteNotice(false);
            }
    ...
    const completeRegistration = async () => {
        if (!canRegister || !registrationOtpSent || registrationConfirmation === null || registrationInFlight.current || props.pending)
            return;
        ...
        try {
            ...
            if (result.value.status === 'join-required') {
                setOtp('');
                setJoinContext({ fullHandle: result.value.fullHandle, phone: phone.trim(), ...result.value.methodCapabilities === undefined ? {} : { methodCapabilities: result.value.methodCapabilities } });
                return;
            }
            setPhone('');
            setHandle('');
    ...
    useEffect(() => {
        ...
        setJoinContext(choice === null ? null : { fullHandle: choice.fullHandle, phone, ...choice.methodCapabilities === undefined ? {} : { methodCapabilities: choice.methodCapabilities } });
    }, [props.access]);
    ...
    if (props.sessionStatus === 'recovery-required' && props.access?.methodCapabilities?.handleRecovery === false) {
        return _jsxs(AwikiIdentityPage, { children: [_jsx("p", { role: "status", children: "\u6B64\u8EAB\u4EFD\u4E0D\u652F\u6301\u6062\u590D\u3002Web \u9996\u4E2A\u7BA1\u7406\u5458\u4E22\u5931\u540E\u65E0\u6CD5\u6062\u590D\u7BA1\u7406\u80FD\u529B\uFF1B\u53EF\u4F7F\u7528\u4ECD\u6709\u6548\u7684\u8BBE\u5907\u7EE7\u7EED\u8BBF\u95EE\u3002" }), _jsx("button", { type: "button", className: css.secondary, disabled: props.accessLoading, onClick: () => { void props.refreshIdentityAccess?.(); }, children: "\u91CD\u65B0\u68C0\u67E5\u8EAB\u4EFD\u72B6\u6001" })] });
    }
    ...
    if (props.sessionStatus === 'unregistered' && joinContext !== null) {
        return (_jsx(AwikiIdentityPage, { children: _jsxs("div", { className: css.accessFlow, children: [_jsx("div", { className: css.identityIcon, children: _jsx(IconUserOutline16, { size: 24 }) }), _jsxs("div", { className: css.headingGroup, children: [_jsx("h3", { children: "\u8FD9\u4E2A Handle \u5DF2\u5B58\u5728" }), _jsx("p", { children: "\u63A8\u8350\u628A\u5F53\u524D DSH \u4F5C\u4E3A\u65B0\u8BBE\u5907\u52A0\u5165\uFF0C\u539F\u8EAB\u4EFD\u548C\u5176\u4ED6\u8BBE\u5907\u4F1A\u7EE7\u7EED\u6709\u6548\u3002" })] }), _jsx("button", { type: "button", className: css.primary, disabled: props.pending, onClick: () => { void beginJoin(); }, children: "\u52A0\u5165\u65B0\u8BBE\u5907\uFF08\u63A8\u8350\uFF09" }), joinContext.methodCapabilities?.handleRecovery === true && _jsx("button", { type: "button", className: css.dangerLink, disabled: props.pending, onClick: chooseRecovery, children: "\u6062\u590D Handle\uFF08\u4F1A\u66FF\u6362 DID\uFF09" }), joinContext.methodCapabilities?.method === 'web' && _jsx("p", { className: css.notice, children: "Web \u8EAB\u4EFD\u4E0D\u652F\u6301\u6062\u590D\u6216\u7BA1\u7406\u6743\u8F6C\u79FB\uFF1B\u9996\u4E2A\u7BA1\u7406\u5458\u4E22\u5931\u540E\u65E0\u6CD5\u6062\u590D\u7BA1\u7406\u80FD\u529B\u3002\u8BF7\u4F7F\u7528\u5DF2\u6709\u7BA1\u7406\u8BBE\u5907\u6279\u51C6\u52A0\u5165\u3002" }), _jsx("button", { type: "button", className: css.secondary, disabled: props.pending, onClick: () => { void cancelJoin(); }, children: "\u53D6\u6D88" }), error !== null && _jsx("small", { className: css.error, role: "alert", children: error })] }) }));
    }
    if (props.sessionStatus === 'signed-out') {
        if (signedOutRecoveryOpen) {
    ...
    return (_jsxs(AwikiIdentityPage, { ...registrationOtpSent ? { onBack: resetIdentityEntry, backLabel: '修改身份信息' } : {}, backDisabled: props.pending, children: [_jsxs("form", { className: css.accessFlow, onSubmit: (event) => {
                ...
                }, children: [_jsx("div", { className: css.identityIcon, children: _jsx(IconUserOutline16, { size: 24 }) }), _jsxs("div", { className: css.headingGroup, children: [_jsx("h3", { children: registrationOtpSent ? '验证身份' : deviceRejoinHandle === null ? '进入 AWiki' : '重新加入设备' }), _jsx("p", { children: registrationOtpSent
                                    ? '输入注册验证码。新 Handle 会创建身份，已有 Handle 会进入设备加入选择。'
                                    : deviceRejoinHandle === null
                                        ? '输入 Handle 和手机号继续。已有恢复进度会直接继续，无需重复获取验证码。'
                                        : '验证绑定手机号后，这台设备会以一把新密钥重新申请加入；本机消息数据不会清除。' })] }), !registrationOtpSent && (props.access?.pendingRegistrations?.length ?? 0) > 0 && _jsxs("div", { className: css.actionStack, "aria-label": "\u672A\u5B8C\u6210\u7684\u6CE8\u518C", children: [_jsx("p", { children: "\u9009\u62E9\u539F\u6CE8\u518C\u7EE7\u7EED\uFF1B\u8BF7\u91CD\u65B0\u8F93\u5165\u624B\u673A\u53F7\u548C\u6709\u6548\u9A8C\u8BC1\u7801\u3002" }), props.access?.pendingRegistrations?.filter(value => value.verificationKind === 'phone').map(value => _jsxs("button", { type: "button", className: css.secondary, disabled: entryPending || props.pending, onClick: () => { setPendingRegistration(value); setHandle(value.fullHandle); setDidMethod(value.method); setOtp(''); setNotice('将继续原注册，保留同一个 DID。'); }, children: ["\u7EE7\u7EED\u6CE8\u518C ", value.fullHandle] }, value.did))] }), (creationMethods.length > 1 || (creationMethods.length === 1 && creationMethods[0] === 'web')) && _jsxs("label", { className: css.field, children: ["\u8EAB\u4EFD\u65B9\u6CD5", _jsx("select", { "aria-label": "\u8EAB\u4EFD\u65B9\u6CD5", value: didMethod, disabled: entryPending || registrationOtpSent || props.pending, onChange: event => { setDidMethod(event.target.value); setPendingRegistration(null); }, children: creationMethods.map(method => _jsx("option", { value: method, children: method === 'wba' ? 'WBA（默认）' : 'Web' }, method)) })] }), didMethod === 'web' && _jsx("p", { className: css.notice, children: "Web \u8EAB\u4EFD\u652F\u6301\u72EC\u7ACB\u8BBE\u5907\u52A0\u5165\u548C\u64A4\u9500\uFF0C\u4E0D\u652F\u6301\u6062\u590D\u6216\u7BA1\u7406\u6743\u8F6C\u79FB\u3002\u9996\u4E2A\u7BA1\u7406\u5458\u4E22\u5931\u540E\u65E0\u6CD5\u6062\u590D\u7BA1\u7406\u80FD\u529B\u3002" }), !canRegister && _jsx("p", { className: css.notice, children: "\u5F53\u524D\u670D\u52A1\u6682\u4E0D\u63A5\u53D7\u6240\u9009\u65B9\u6CD5\u7684\u65B0\u6CE8\u518C\u3002\u53EF\u7EE7\u7EED\u5DF2\u4FDD\u5B58\u7684\u6CE8\u518C\uFF0C\u6216\u91CD\u65B0\u68C0\u67E5\u3002" }), _jsxs("label", { className: css.field, children: ["Handle", _jsx("input", { value: handle, onChange: event => { setHandle(event.target.value); setPendingRegistration(null); setShortHandleInviteNotice(false); setInviteCode(''); setError(null); }, readOnly: entryPending || props.pending || registrationOtpSent || deviceRejoinHandle !== null, autoComplete: "username", placeholder: "\u4F8B\u5982 alice", autoFocus: props.autoFocusHandle })] }), _jsxs("label", { className: css.field, children: ["\u624B\u673A\u53F7", _jsx("input", { value: phone, onChange: event => { setPhone(event.target.value); }, readOnly: entryPending || props.pending || registrationOtpSent, type: "tel", autoComplete: "tel" })] }), shortHandleInviteNotice && _jsxs("label", { className: css.field, children: ["\u9080\u8BF7\u7801", _jsx("input", { value: inviteCode, onChange: event => { setInviteCode(event.target.value); setError(null); }, readOnly: entryPending || props.pending || registrationOtpSent, autoComplete: "off", maxLength: 512, placeholder: "\u8BF7\u8F93\u5165\u9080\u8BF7\u7801" })] }), registrationOtpSent && _jsxs("label", { className: css.field, children: ["\u6CE8\u518C\u9A8C\u8BC1\u7801", _jsx("input", { value: otp, onChange: event => { setOtp(event.target.value); }, inputMode: "numeric", autoComplete: "one-time-code", autoFocus: true })] }), _jsx("button", { type: "submit", className: css.primary, disabled: entryPending || props.pending || props.accessLoading || (shortHandleInviteNotice && inviteCode.trim() === '') || handle.trim() === '' || phone.trim() === '' || (registrationOtpSent && otp.trim() === ''), children: registrationOtpSent ? '继续' : '获取验证码' }), registrationOtpSent && (_jsx("button", { type: "button", className: css.linkButton, disabled: entryPending || props.pending || retrySeconds > 0, onClick: () => { void requestRegistrationOtp(); }, children: retrySeconds > 0 ? `${retrySeconds} 秒后重新获取` : '重新获取注册验证码' })), shortHandleInviteNotice && _jsx("div", { className: css.inviteNotice, role: "alert", children: SHORT_HANDLE_INVITE_MESSAGE }), notice !== null && _jsx("small", { className: css.notice, role: "status", children: notice }), error !== null && _jsx("small", { className: css.error, role: "alert", children: error })] }), _jsxs(Modal, { open: registrationConfirmation !== null, title: "\u786E\u8BA4\u521B\u5EFA AWiki \u8EAB\u4EFD", description: "\u65B0 Handle \u5C06\u901A\u8FC7 ANP Identity \u521B\u5EFA\u8EAB\u4EFD\u5E76\u6CE8\u518C\u5230 AWiki\uFF1B\u5982\u679C Handle \u5DF2\u5B58\u5728\uFF0C\u5C06\u7EE7\u7EED\u8BBE\u5907\u52A0\u5165\u6D41\u7A0B\u3002", closeLabel: "\u53D6\u6D88", onClose: () => { if (!registrationInFlight.current)
                    setRegistrationConfirmation(null); }, footer: _jsxs(_Fragment, { children: [_jsx(Button, { type: "button", variant: "outline", disabled: registrationSubmitting || props.pending, onClick: () => { if (!registrationInFlight.current)
                                setRegistrationConfirmation(null); }, children: "\u8FD4\u56DE\u4FEE\u6539" }), _jsx(Button, { type: "button", disabled: registrationSubmitting || props.pending, onClick: () => { void completeRegistration(); }, children: registrationSubmitting ? '正在提交…' : '确认并继续' })] }), children: [_jsxs("p", { children: ["Handle\uFF1A", registrationConfirmation?.handle] }), _jsx("p", { children: "\u8EAB\u4EFD\u5BC6\u94A5\u7531\u672C\u673A ANP Identity \u4FDD\u7BA1\u3002\u53D6\u6D88\u4E0D\u4F1A\u63D0\u4EA4\u6CE8\u518C\u6216\u521B\u5EFA\u8EAB\u4EFD\u3002" })] })] }));
}
