...
export function AwikiIdentityAccess(props) {
    ...
    const completeRegistration = async () => {
        if (!registrationOtpSent || registrationConfirmation === null || registrationInFlight.current || props.pending)
            return;
    ...
    return (_jsxs(AwikiIdentityPage, { ...registrationOtpSent ? { onBack: resetIdentityEntry, backLabel: '修改身份信息' } : {}, backDisabled: props.pending, children: [_jsxs("form", { className: css.accessFlow, onSubmit: (event) => {
                    ...
                    if (registrationOtpSent) {
                        setRegistrationConfirmation({ phone: phone.trim(), handle: handle.trim(), otp: otp.trim(), ...(inviteCode.trim() === '' ? {} : { inviteCode: inviteCode.trim() }) });
                    }
                    else {
