...
import { shortHandleInviteRequired, SHORT_HANDLE_INVITE_MESSAGE } from "../registration-policy.js";
...
export function AwikiIdentityAccess(props) {
    ...
    const requestIdentityOtp = async () => {
        ...
        if (shortHandleInviteRequired(handle)) {
            ...
            if (!inspection.ok)
                return setError(inspection.error);
    ...
    if (!recoveryOpen && ((props.access === null && props.accessLoading) || (props.access === null && props.accessError))) {
        return _jsxs(AwikiIdentityPage, { children: [_jsx("p", { role: "status", children: props.accessError ?? '正在确认本机身份操作…' }), props.accessError && _jsx("button", { type: "button", className: css.primary, disabled: props.accessLoading, onClick: () => { void props.refreshIdentityAccess?.(); }, children: "\u91CD\u65B0\u68C0\u67E5\u8EAB\u4EFD\u72B6\u6001" })] });
    }
    if (props.accessError && !recoveryOpen) {
        return _jsxs(AwikiIdentityPage, { children: [_jsx("p", { role: "alert", children: props.accessError }), _jsx("button", { type: "button", className: css.primary, onClick: () => { void props.refreshIdentityAccess?.(); }, children: "\u91CD\u65B0\u68C0\u67E5\u8EAB\u4EFD\u72B6\u6001" })] });
    }
    ...
    return (_jsxs(AwikiIdentityPage, { ...registrationOtpSent ? { onBack: resetIdentityEntry, backLabel: '修改身份信息' } : {}, backDisabled: props.pending, children: [props.sessionStatus === 'recovery-required' && !registrationOtpSent && (_jsxs("div", { className: css.headingGroup, children: [_jsx("h3", { children: "\u9700\u8981\u91CD\u65B0\u6062\u590D\u8EAB\u4EFD" }), _jsx("p", { children: "\u539F\u8EAB\u4EFD\u5DF2\u5728\u5176\u4ED6\u8BBE\u5907\u66F4\u65B0\uFF0C\u672C\u673A\u65E7\u51ED\u8BC1\u5DF2\u5931\u6548\u3002\u53EF\u4EE5\u6062\u590D\u539F\u8EAB\u4EFD\uFF0C\u4E5F\u53EF\u4EE5\u5728\u4E0B\u65B9\u4F7F\u7528\u5176\u4ED6\u8D26\u53F7\uFF1B\u8FD4\u56DE\u4E0D\u4F1A\u6E05\u9664\u672C\u673A\u6570\u636E\u3002" }), revokedHandle !== undefined && _jsxs("div", { className: css.identitySummary, children: [_jsx("span", { children: "\u539F\u8EAB\u4EFD" }), _jsx("strong", { children: revokedHandle })] }), revokedHandle !== undefined && props.handleRecoveryPhoneEnabled && _jsx("button", { type: "button", className: css.secondary, disabled: props.pending, onClick: () => { setRecoveryChoice(revokedHandle); }, children: "\u6062\u590D\u672C\u673A\u539F\u6709\u8EAB\u4EFD" })] })), _jsxs("form", { className: css.accessFlow, onSubmit: (event) => { event.preventDefault(); void (registrationOtpSent ? completeRegistration() : requestIdentityOtp()); }, children: [_jsx("div", { className: css.identityIcon, children: _jsx(IconUserOutline16, { size: 24 }) }), _jsxs("div", { className: css.headingGroup, children: [_jsx("h3", { children: registrationOtpSent ? '验证身份' : deviceRejoinHandle === null ? '进入 AWiki' : '重新加入设备' }), _jsx("p", { children: registrationOtpSent
