...
export class AwikiSettingsController {
    ...
    async loadDesktopUpdate(refresh = false) {
        ...
        try {
            ...
            if (this.disposed || request !== this.desktopRequestVersion || tenantId !== this.tenantSnapshot.value.activeTenantId
                || generation !== this.tenantSnapshot.value.generation)
                return;
            if (!result.ok)
            ...
            if (desktop?.schemaVersion === 2 && desktop.tenantId !== undefined) {
                ...
                if (desktop.tenantId !== tenantId || desktop.tenantGeneration !== generation
                    || desktop.policyOrigin !== tenant?.backendBaseUrl)
                    throw new Error('Desktop update scope mismatch');
            }
    ...
    async writeTenant(endpoint, payload) {
        ...
        this.publishTenants({ ...this.tenantSnapshot, ...switching ? { update: undefined, updateStatus: 'loading',
                desktop: undefined, desktopStatus: 'loading' } : {}, value: { ...this.tenantSnapshot.value, switching: endpoint === AWIKI_SETTINGS_RPC_ENDPOINTS.switchTenant } });
        try {
