...
export function AwikiOnboarding(props) {
    ...
    if (identity.status === 'cold' || identity.status === 'loading' || models.account?.enabled === true)
        return null;
    const skip = _jsx(Button, { type: "button", variant: "outline", onClick: props.complete, children: t('onboardingUseApiKey') });
    if (identity.status === 'error') {
        return (_jsxs(OnboardingModal, { title: t('onboardingConnectTitle'), children: [_jsx("p", { className: css.description, children: identity.error ?? t('onboardingIdentityUnavailable') }), _jsx("div", { className: css.actions, children: skip })] }));
    }
    if (identity.sessionStatus === 'unregistered') {
        return (_jsxs(OnboardingModal, { title: t('onboardingModelTitle'), children: [_jsx("p", { className: css.description, children: t('onboardingRegistrationDescription') }), _jsx(AwikiRegistrationForm, { pending: identity.pending !== null, sendRegistrationOtp: request => props.identity.sendRegistrationOtp(request), registerIdentity: request => props.identity.registerIdentity(request) }), _jsx("div", { className: css.actions, children: skip })] }));
    }
    if (identity.sessionStatus === 'signed-out') {
        return (_jsxs(OnboardingModal, { title: t('onboardingRestoreTitle'), children: [_jsx("p", { className: css.description, children: t('onboardingRestoreDescription') }), _jsxs("div", { className: css.actions, children: [_jsx(Button, { type: "button", disabled: identity.pending !== null, onClick: () => { void props.identity.login(); }, children: t('onboardingRestore') }), skip] })] }));
    }
    if (models.status === 'idle' || models.status === 'loading')
        return null;
    const account = models.account?.account;
    return (_jsxs(OnboardingModal, { title: t('onboardingEnableTitle'), children: [models.status === 'unavailable' || account === undefined ? (_jsx("p", { className: css.error, role: "alert", children: models.error ?? t('modelAccountUnavailable') })) : (_jsxs(_Fragment, { children: [_jsxs("div", { className: css.accountRow, children: [_jsx("span", { children: t('accountBalance') }), _jsxs("strong", { children: [account.balance, " ", account.currency] })] }), _jsx("p", { className: css.description, children: account.billing_mode === 'development_bypass'
...
function OnboardingModal({ title, children }) {
    ...
    return (_jsx(Modal, { open: true, title: title, onClose: () => { }, headless: true, className: css.dialog ?? '', children: _jsxs("div", { className: css.content, children: [_jsx("h2", { ref: titleRef, tabIndex: -1, className: css.title, children: title }), children] }) }));
}
