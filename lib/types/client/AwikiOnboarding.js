...
export function AwikiOnboarding(props) {
    ...
    useEffect(() => {
        if (availability.status === 'unavailable' || (availability.status === 'ready' && availability.usable))
            props.complete();
    ...
    if (!shouldOffer || identity.status === 'cold' || identity.status === 'loading' || models.account?.enabled === true)
        return null;
    ...
    return (_jsxs(OnboardingModal, { title: t('onboardingEnableTitle'), closeLabel: t('onboardingClose'), onClose: dismiss, children: [models.status === 'unavailable' || account === undefined ? (_jsx("p", { className: css.error, role: "alert", children: models.error ?? t('modelAccountUnavailable') })) : (_jsxs(_Fragment, { children: [_jsxs("div", { className: css.accountRow, children: [_jsx("span", { children: t('accountBalance') }), _jsxs("strong", { children: [account.balance, " ", account.currency] })] }), _jsx("p", { className: css.description, children: account.billing_mode === 'development_bypass'
                            ...
                            : pendingOrder !== null
                                ...
                                : account.model_access_reason === 'insufficient_balance'
                                    ...
                                    : t('onboardingStrictDescription') }), !account.payments_available && _jsx("p", { className: css.notice, children: t('paymentsUnavailable') })] })), _jsxs("div", { className: css.actions, children: [pendingOrder !== null ? (_jsx(Button, { type: "button", onClick: openAccountSettings, children: t('continuePayment') })) : accessUnavailable ? (_jsx(Button, { type: "button", disabled: account?.payments_available !== true, onClick: openAccountSettings, children: t('goToRecharge') })) : account !== undefined ? (_jsx(Button, { type: "button", disabled: models.pending !== null || models.status === 'loading', onClick: () => { void props.models.setEnabled(true); }, children: models.pending === 'enable' ? t('enablingModels') : t('enableModels') })) : null, alternatives] })] }));
}
