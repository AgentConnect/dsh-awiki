...
function recipient(summary, folder, account, selectedSummary) {
    const fallback = folder === 'inbox' ? account?.mailboxAddress ?? '未提供' : '未提供';
    const to = summary.to.length > 0
        ? summary.to
        : folder === 'sent' && selectedSummary?.id === summary.id ? selectedSummary.to : [];
    return participant(to, fallback);
}
...
export function AwikiMail(props) {
    ...
    const downloadAttachment = async (attachment) => {
        ...
        try {
            const saved = await downloadAndSaveMailAttachment(props.downloadMailAttachment, { localMessageId, attachmentIndex: attachment.index }, expected, downloadMaxBytes, () => generation === detailGeneration.current && message.summary.id === localMessageId);
            if (!saved)
                return;
