...
export class AwikiController {
    ...
    async open() {
        ...
        const identities = identity === null
            ? { ok: true, value: { items: Object.freeze([]) } }
            : await call(() => this.remote.listIdentities());
        if (!this.current(generation))
            return { ok: true, value: undefined };
        if (!identities.ok)
            return this.fail(identities.error);
    ...
    async selectIdentity(identityId) {
        if (this.view.activeIdentityId === identityId)
            return { ok: true, value: undefined };
        const tab = this.view.identities.find(item => item.identity.identityId === identityId);
        if (tab === undefined || tab.status === 'broken')
            return this.fail('该 AWiki 身份当前不可用');
        this.saveActiveTab();
        this.close();
        const generation = this.generation;
    ...
    async registerIdentity(request) {
        ...
        if (!identities.ok || !this.current(generation))
            return result;
        this.publish({
            ...this.view,
    ...
    async loadMoreConversations() {
        const generation = this.generation;
        const result = await this.withPending('加载更多会话', () => call(() => this.remote.listConversations({ ...this.conversationsCursor === undefined ? {} : { cursor: this.conversationsCursor }, ...this.scope() })));
        if (!result.ok)
            return result;
        if (!this.current(generation))
            return { ok: true, value: undefined };
    ...
    startTimer(generation) {
        if (this.config === null || !this.current(generation))
            return;
        if (this.timer !== undefined)
            clearInterval(this.timer);
