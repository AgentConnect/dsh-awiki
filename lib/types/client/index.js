...
import { clearIntegrationOperation, clearIntegrationOperations, durableIntegrationOperationId, } from "./integration-operation.js";
...
async function durableIntegrationMutation(kind, signature, operation) {
    ...
    if (result.ok)
        clearIntegrationOperation(kind);
    return result;
}
...
export async function apply(ctx) {
    ...
    try {
        ...
        const injectedSettings = () => ({
            ...
            saveIntegration: (fields, current) => current === null
                ? durableIntegrationMutation('create', JSON.stringify(fields), idempotencyKey => awiki.createIntegration({ ...fields, idempotencyKey }))
                : durableIntegrationMutation('update', JSON.stringify({ owner: current.owner.handle, revision: current.revision, fields }), idempotencyKey => awiki.updateIntegration({ ...fields, expectedRevision: current.revision, idempotencyKey })),
            rotateIntegrationId: current => durableIntegrationMutation('rotate', `${current.owner.handle}:${current.id}:${current.revision}`, idempotencyKey => awiki.rotateIntegrationId({ expectedRevision: current.revision, idempotencyKey })),
            closeIntegration: current => durableIntegrationMutation('close', `${current.owner.handle}:${current.id}:${current.revision}`, idempotencyKey => awiki.closeIntegration({ expectedRevision: current.revision, idempotencyKey })),
            reopenIntegration: (fields, current) => durableIntegrationMutation('reopen', JSON.stringify({ owner: current.owner.handle, revision: current.revision, fields }), idempotencyKey => awiki.reopenIntegration({
                ...fields,
