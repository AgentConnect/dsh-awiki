...
import { AwikiDeviceJoinReminder } from "./AwikiDeviceJoinReminder.js";
...
function Chat(props) {
    ...
    const sendMessage = async () => {
        ...
        if (!mounted.current || selectedConversationId.current !== conversationId)
            return;
        const messageId = `msg-${crypto.randomUUID()}`;
        setSendingDraft({
            conversationId,
...
export function AwikiOverlay(props) {
    return _jsx(AwikiDraftProvider, { ...props.drafts === undefined ? {} : { store: props.drafts }, children: _jsx(AwikiOverlayContent, { ...props }) });
}
