...
function Chat(props) {
    ...
    return (_jsxs("div", { className: css.identityChat, children: [_jsx("nav", { className: css.identityTabs, "aria-label": "AWiki \u8EAB\u4EFD", children: view.identities.map(tab => (_jsxs("button", { type: "button", className: css.identityTab, "data-active": tab.identity.identityId === view.activeIdentityId || undefined, disabled: view.pending !== null || tab.status === 'broken', title: `${tab.identity.handle}\n${tab.identity.did}`, onClick: () => { void props.selectIdentity(tab.identity.identityId); }, children: [_jsx("span", { children: tab.identity.isDefault ? `主身份 · ${tab.displayName}` : tab.displayName }), tab.status === 'unbound' && _jsx("small", { children: "\u672A\u5173\u8054" })] }, tab.tabId))) }), _jsxs("div", { className: css.chat, children: [_jsxs("aside", { className: css.roster, "data-hidden": selected !== undefined || undefined, children: [_jsx(IdentityCard, { identity: view.identity, pending: view.pending !== null, updateDisplayName: props.updateDisplayName }), _jsx("div", { className: css.rosterTitle, children: "\u4F1A\u8BDD" }), _jsxs("div", { className: css.conversationList, children: [view.conversations.map(conversation => (_jsx(ConversationRow, { conversation: conversation, active: conversation.id === view.selectedConversationId, onSelect: () => { void props.selectConversation(conversation.id); } }, conversation.id))), view.conversations.length === 0 && _jsx("p", { className: css.empty, children: view.identity.isDefault ? '还没有可用的私聊或群聊。' : '该 Agent 身份还没有私聊。' })] }), view.conversationsHasMore && _jsx("button", { type: "button", className: css.more, onClick: () => { void props.loadMoreConversations(); }, children: "\u52A0\u8F7D\u66F4\u591A\u4F1A\u8BDD" })] }), _jsx("section", { className: css.thread, "data-visible": selected !== undefined || undefined, children: selected === undefined ? (_jsxs("div", { className: css.threadEmpty, children: [_jsx(IconGlobeOutline14, { size: 28 }), _jsx("p", { children: view.identity.isDefault ? '选择一个私聊或群聊查看消息。' : '选择一个私聊查看消息。' })] })) : (_jsxs(_Fragment, { children: [_jsxs("header", { className: css.threadHeader, children: [_jsx("button", { type: "button", className: css.back, "aria-label": "\u8FD4\u56DE\u4F1A\u8BDD\u5217\u8868", onClick: () => { void props.selectConversation(null); }, children: _jsx(IconChevronLeftOutline14, {}) }), _jsxs("div", { className: css.threadTitle, children: [_jsx("strong", { children: conversationLabel(selected) }), _jsx("small", { children: selected.kind === 'direct' ? '私聊' : '群聊' })] }), _jsxs("button", { type: "button", className: css.summaryTrigger, "aria-controls": summaryPanelId, "aria-expanded": summary === undefined ? undefined : !summary.collapsed, "aria-label": summary?.status === 'loading' ? '正在生成 AI 总结' : summary?.collapsed === true ? '展开 AI 总结' : '生成 AI 总结', disabled: summary?.status === 'loading', onClick: () => {
...
export function AwikiOverlay(props) {
    ...
    const rememberedConversationIds = useRef(new Map());
    ...
    const selectConversation = (conversationId) => {
        ...
        if (conversationId === null)
            rememberedConversationIds.current.delete(key);
        else
            rememberedConversationIds.current.set(key, conversationId);
