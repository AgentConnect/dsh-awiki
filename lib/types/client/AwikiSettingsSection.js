...
import { AwikiUpdates } from "./AwikiUpdates.js";
import { AwikiDraftProvider, useDraftState } from "./drafts.js";
...
export function AwikiSettingsSection(props) {
    return _jsx(AwikiDraftProvider, { store: props.drafts, children: _jsx(AwikiSettingsContent, { ...props }) });
}
function AwikiSettingsContent(props) {
    ...
    useEffect(() => {
        if (restricted && tab !== 'tenants' && tab !== 'updates')
            setTab('updates');
    }, [restricted, tab]);
    useEffect(() => {
        if (!restricted && (tab === 'devices' || tab === 'integration') && awiki.status === 'cold')
            void props.loadAwiki();
    }, [awiki.status, props.loadAwiki, restricted, tab]);
    return (_jsxs("section", { className: css.section, children: [_jsxs("div", { className: css.heading, children: [_jsx("h2", { className: css.title, children: props.t('nav') }), _jsx("p", { className: css.intro, children: props.t('intro') })] }), restricted && _jsxs("p", { className: css.error, role: "alert", children: [props.t('updateRequired'), " ", _jsx(Button, { type: "button", variant: "outline", onClick: () => { setTab('updates'); }, children: props.t('updatesTab') })] }), _jsx("div", { className: css.tabs, role: "tablist", "aria-label": props.t('tabsLabel'), children: tabs.map(item => _jsxs("button", { type: "button", role: "tab", "aria-selected": tab === item.id, className: `${css.tab} ${tab === item.id ? css.tabActive : ''}`, onClick: () => { setTab(item.id); }, children: [item.label, item.id === 'updates' && (tenantSnapshot.update?.updateAvailable || tenantSnapshot.desktop?.updateAvailable || restricted) && _jsx("span", { "aria-hidden": "true", children: " \u25CF" })] }, item.id)) }), _jsxs("div", { role: "tabpanel", children: [tab === 'tenants' && _jsx(TenantPanel, { ...props }), tab === 'updates' && _jsx(AwikiUpdates, { ...props, snapshot: tenantSnapshot }), !restricted && tab === 'devices' && (awiki.status === 'cold' || awiki.status === 'loading'
                        ...
                        : awiki.status === 'error'
                            ...
                            : awiki.sessionStatus === 'active'
                                ? _jsx(AwikiDevices, { active: true, pending: awiki.pending !== null, refreshDeviceManagement: props.refreshDeviceManagement, ...props.getIdentityServices === undefined ? {} : { getIdentityServices: props.getIdentityServices }, ...props.updateIdentityServices === undefined ? {} : { updateIdentityServices: props.updateIdentityServices }, ...props.resumeIdentityServicesUpdate === undefined ? {} : { resumeIdentityServicesUpdate: props.resumeIdentityServicesUpdate }, startDeviceJoinVerification: props.startDeviceJoinVerification, approveDeviceJoin: props.approveDeviceJoin, rejectDeviceJoin: props.rejectDeviceJoin, revokeDevice: props.revokeDevice, retryDeviceManagement: props.retryDeviceManagement, prepareRootTransfer: props.prepareRootTransfer, confirmRootTransfer: props.confirmRootTransfer }, `${tenantSnapshot.status === 'ready' ? `${tenantSnapshot.value.activeTenantId}:${tenantSnapshot.value.generation}` : 'unavailable'}:${awiki.identity?.did ?? 'none'}`)
                                : _jsx("p", { className: css.notice, children: props.t('devicesUnavailable') })), !restricted && tab === 'local' && _jsx(LocalDataPanel, { ...props, disabled: tenantSnapshot.status !== 'ready' || tenantSnapshot.value.switching }, tenantSnapshot.status === 'ready' ? `${tenantSnapshot.value.activeTenantId}:${tenantSnapshot.value.generation}:${tenantSnapshot.value.switching}` : 'unavailable'), !restricted && tab === 'integration' && (tenantSnapshot.status !== 'ready' || tenantSnapshot.value.switching || awiki.status === 'cold' || awiki.status === 'loading'
                        ? _jsx("p", { className: css.status, role: "status", children: props.t('integrationLoading') })
                        : _jsx(AwikiIntegrationSettings, { ...props }, `${tenantSnapshot.status === 'ready' ? tenantSnapshot.value.activeTenantId : 'unavailable'}:${awiki.identity?.did ?? 'no-identity'}`))] })] }));
}
...
function LocalDataPanel(props) {
    ...
    const clear = async () => {
        if (props.disabled || clearDraft !== props.t('clearConfirmationPhrase'))
            return;
        setClearing(true);
        setStatus(null);
        try {
