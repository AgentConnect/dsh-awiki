...
export function AwikiSettingsSection(props) {
    ...
    return (_jsxs("section", { className: css.section, children: [_jsxs("div", { className: css.heading, children: [_jsx("h2", { className: css.title, children: props.t('nav') }), _jsx("p", { className: css.intro, children: props.t('intro') })] }), _jsx("div", { className: css.tabs, role: "tablist", "aria-label": props.t('nav'), children: tabs.map(item => _jsx("button", { type: "button", role: "tab", "aria-selected": tab === item.id, className: `${css.tab} ${tab === item.id ? css.tabActive : ''}`, onClick: () => { setTab(item.id); }, children: item.label }, item.id)) }), _jsxs("div", { role: "tabpanel", children: [tab === 'tenants' && _jsx(TenantPanel, { ...props }), tab === 'local' && _jsx(LocalDataPanel, { ...props }), tab === 'integration' && _jsx(AwikiIntegrationSettings, { ...props })] })] }));
}
...
function TenantRow(props) {
    ...
    return (_jsxs("article", { className: `${css.tenantRow} ${current ? css.tenantCurrent : ''}`, children: [_jsxs("div", { className: css.tenantCopy, children: [props.tenant.kind === 'custom' ? _jsx("input", { "aria-label": props.t('tenantName'), className: css.inlineInput, value: draftName, disabled: props.disabled, maxLength: 80, onChange: event => { setDraftName(event.target.value); } }) : _jsx("strong", { children: props.tenant.displayName }), _jsx("span", { className: css.tenantMeta, children: props.tenant.didHost }), _jsxs("span", { className: css.tenantBadges, children: [_jsx("span", { children: props.tenant.kind === 'built_in' ? props.t('tenantOfficial') : props.t('tenantCustom') }), current && _jsx("span", { children: props.t('tenantCurrent') })] })] }), _jsxs("div", { className: css.actions, children: [!current && _jsx(Button, { type: "button", disabled: props.disabled, onClick: () => { void run(() => props.switchTenant(props.tenant.tenantId), props.t('tenantSwitched')); }, children: props.t('tenantSwitch') }), props.tenant.kind === 'custom' && _jsxs(_Fragment, { children: [_jsx(Button, { type: "button", variant: "outline", disabled: props.disabled || draftName.trim() === '' || draftName === props.tenant.displayName, onClick: () => { void run(() => props.renameTenant(props.tenant.tenantId, draftName.trim()), props.t('tenantRenamed')); }, children: props.t('save') }), !current && _jsx(Button, { type: "button", variant: "outline", disabled: props.disabled, onClick: () => { void run(() => props.archiveTenant(props.tenant.tenantId), props.t('tenantArchived')); }, children: props.t('tenantArchive') })] })] })] }));
}
