...
export function AwikiRecoveryForm(props) {
    ...
    const postAttemptFactorRefresh = props.progress?.phase === 'remote_outcome_unknown'
        && props.progress.failureCode === 'factor_retry_required';
    useEffect(() => {
    ...
    const resendOtp = async () => {
        if (effectiveFactorContext === null)
            return;
    ...
    if (props.progress === null || props.progress.phase === 'awaiting_factor' || postAttemptFactorRefresh) {
        return (_jsx(AwikiIdentityPage, { onBack: () => { void discard(); }, backLabel: "\u53D6\u6D88\u6062\u590D", backDisabled: props.pending || commitAttempted, children: _jsxs("form", { className: css.recoveryForm, onSubmit: (event) => { event.preventDefault(); void prepare(); }, children: [_jsx("div", { className: css.recoveryStatusLine, children: _jsx("span", { children: "\u6062\u590D\u8BF7\u6C42\u5DF2\u521B\u5EFA" }) }), _jsx("h3", { children: postAttemptFactorRefresh ? '重新验证身份归属' : '验证身份归属' }), _jsx("p", { children: postAttemptFactorRefresh
                            ? '服务端尚未记录恢复结果，原验证授权已过期。请为同一个恢复操作重新获取验证码。'
                            : '验证码已发送，请完成验证后再确认是否恢复。' }), _jsxs("div", { className: css.recoveryIdentitySummary, children: [_jsx("span", { children: "\u6062\u590D\u8EAB\u4EFD" }), _jsx("strong", { children: effectiveFactorContext?.fullHandle ?? props.progress?.fullHandle ?? '待确认' }), effectiveFactorContext !== null && _jsxs(_Fragment, { children: [_jsx("span", { children: "\u9A8C\u8BC1\u7801\u5DF2\u53D1\u9001\u81F3" }), _jsx("strong", { children: maskedPhone(effectiveFactorContext.phone) })] })] }), effectiveFactorContext === null && (_jsxs("label", { children: ["\u7ED1\u5B9A\u624B\u673A\u53F7", _jsx("input", { ref: factorPhone, type: "tel", autoComplete: "tel", autoFocus: true })] })), postAttemptFactorRefresh && (_jsx("button", { type: "button", className: css.secondary, disabled: props.pending, onClick: () => { void refreshFactorOtp(); }, children: "\u91CD\u65B0\u83B7\u53D6\u6062\u590D\u9A8C\u8BC1\u7801" })), _jsxs("label", { children: ["\u6062\u590D\u9A8C\u8BC1\u7801", _jsx("input", { ref: otp, inputMode: "numeric", autoComplete: "one-time-code", autoFocus: effectiveFactorContext !== null, disabled: postAttemptFactorRefresh && effectiveFactorContext === null })] }), _jsx("button", { type: "submit", className: css.primary, disabled: props.pending || (postAttemptFactorRefresh && effectiveFactorContext === null), children: "\u9A8C\u8BC1\u6062\u590D\u4FE1\u606F" }), !postAttemptFactorRefresh && effectiveFactorContext !== null && (_jsx("button", { type: "button", className: css.secondary, disabled: props.pending, onClick: () => { void resendOtp(); }, children: "\u91CD\u65B0\u83B7\u53D6\u6062\u590D\u9A8C\u8BC1\u7801" })), notice !== null && _jsx("small", { className: css.notice, role: "status", children: notice }), error !== null && _jsx("small", { className: css.inlineError, role: "alert", children: error }), _jsx(RecoveryDiagnostics, { operationId: props.operationId })] }) }));
    }
    const progress = props.progress;
    const preCommit = progress.phase === 'ready_to_commit' && !commitAttempted;
    const continuationAvailable = canResume(progress);
    return (_jsx(AwikiIdentityPage, { ...preCommit ? { onBack: () => { void discard(); }, backLabel: '取消恢复', backDisabled: props.pending } : {}, live: preCommit ? 'off' : 'polite', children: _jsxs("div", { className: css.recoveryForm, children: [_jsx("div", { className: css.recoveryStatusLine, children: _jsx("span", { children: phaseLabel(progress.phase) }) }), _jsx("h3", { children: preCommit ? '确认恢复已有身份' : '身份恢复进度' }), _jsx("p", { className: css.recoveryHandle, children: progress.fullHandle }), _jsx("div", { className: css.recoveryImpact, children: _jsxs("p", { "data-tone": progress.localOrdinaryDataWillMigrate ? 'success' : 'neutral', children: [progress.localOrdinaryDataWillMigrate ? _jsx(IconCheckOutline16, { size: 14 }) : _jsx(IconWarningOutline16, { size: 14 }), "\u666E\u901A\u672C\u5730\u4F1A\u8BDD\u6570\u636E", progress.localOrdinaryDataWillMigrate ? '将迁移到恢复后的身份' : '不会迁移'] }) }), preCommit ? (_jsxs(_Fragment, { children: [_jsx("p", { className: css.recoveryConfirmationCopy, children: "\u786E\u8BA4\u540E\uFF0C\u8FD9\u53F0\u8BBE\u5907\u5C06\u4F7F\u7528\u65B0\u7684\u672C\u673A\u51ED\u8BC1\u3002\u6062\u590D\u5F00\u59CB\u540E\u8BF7\u4FDD\u6301\u7A97\u53E3\u6253\u5F00\uFF0C\u4E0D\u8981\u91CD\u590D\u63D0\u4EA4\u3002" }), _jsx("button", { type: "button", className: css.primary, disabled: props.pending, onClick: () => { void activate(); }, children: "\u786E\u8BA4\u5E76\u6062\u590D\u8EAB\u4EFD" })] })) : (_jsxs("div", { className: css.recoveryProgressPanel, "aria-live": "polite", children: [error === null && progress.phase !== 'quarantined_key_unavailable' && !continuationAvailable && _jsx(IconLoadingOutline16, { size: 18 }), _jsx("p", { children: progressMessage(progress) }), (continuationAvailable || error !== null || progress.phase === 'quarantined_key_unavailable') && (_jsxs("button", { type: "button", className: css.primary, disabled: props.pending, onClick: () => { void (continuationAvailable ? resume() : refresh()); }, children: [_jsx(IconRefreshOutline16, { size: 14 }), progress.phase === 'identity_transition_pending' || progress.phase === 'remote_committed'
                                    ? '继续完成本机切换'
