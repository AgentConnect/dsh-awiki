...
function requireAgent(agent) {
    if (agent === undefined)
        throw new TypeError('AWiki Agent identity tools require a DSH Agent context.');
    return agent;
}
 ...
 */
export function registerAwikiTools(ctx, service) {
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        ...
        execute: (args, exec) => toolResult(service.attachAgentIdentity(requireAgent(exec.agent), {
            ...args.binding_id === undefined ? {} : { bindingId: args.binding_id },
            ...args.identity_id === undefined ? {} : { identityId: args.identity_id },
