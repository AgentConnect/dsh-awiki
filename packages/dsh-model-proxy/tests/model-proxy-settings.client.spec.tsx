...
describe('Model Proxy quick recharge settings', () => {
  ...
  it('shows development access and allows explicit enable while recharge is disabled', async () => {
    ...
    expect(screen.getByRole('heading', { name: '快速充值' })).toBeTruthy()
    expect(screen.getByText('Awiki托管的模型来自DeepSeek官方API，收费标准与DeepSeek官方保持一致')).toBeTruthy()
    const status = screen.getByText('未启用')
    const enable = screen.getByRole('button', { name: '启用' })
    expect(status.closest('dd')?.contains(enable)).toBe(true)
  ...
  it('combines the enabled state with the disable action and preserves explicit opt-out', async () => {
    const current = account()
    const { models } = mount(account({ account: { ...current.account!, enabled: true } }))

    const status = screen.getByText('已启用')
    const disable = screen.getByRole('button', { name: '停用' })
    expect(status.closest('dd')?.contains(disable)).toBe(true)
