...
export function AwikiOnboarding(props) {
    ...
    const identityAccess = (sessionStatus) => (_jsx(IdentityAccess, { sessionStatus: sessionStatus, access: identity.identityAccess, accessLoading: identity.accessLoading, accessError: identity.accessError, refreshIdentityAccess: () => props.identity.refreshIdentityAccess(), selectRecovery: id => props.identity.selectRecovery(id), leaveRecovery: () => props.identity.leaveRecovery(), continueRecoveryForHandle: handle => props.identity.continueRecoveryForHandle(handle), enterRecoveredSession: () => props.identity.enterRecoveredSession(), identity: identity.identity, recoveryOperationId: identity.recoveryOperationId ?? null, recoveryProgress: identity.recoveryProgress ?? null, recoveryOtpRetryAt: identity.recoveryOtpRetryAt, pending: identity.pending !== null, autoFocusHandle: sessionStatus === 'unregistered', handleRecoveryPhoneEnabled: identity.handleRecoveryPhoneEnabled, sendRegistrationOtp: request => props.identity.sendRegistrationOtp(request), registerIdentity: request => props.identity.registerIdentity(request), beginDeviceJoin: () => props.identity.beginDeviceJoin(), getDeviceJoinStatus: () => props.identity.getDeviceJoinStatus(), cancelDeviceJoin: () => props.identity.cancelDeviceJoin(), beginRecoveryFromDeviceJoin: request => props.identity.beginRecoveryFromDeviceJoin(request), retireDeviceIdentityForRejoin: () => props.identity.retireDeviceIdentityForRejoin(), login: () => props.identity.login(), clearLocalIdentity: props.clearLocalIdentity, sendRecoveryOtp: request => props.identity.sendRecoveryOtp(request), prepareRecovery: request => props.identity.prepareRecovery(request), activateRecovery: () => props.identity.activateRecovery(), refreshRecoveryStatus: () => props.identity.refreshRecoveryStatus(), resumeRecovery: () => props.identity.resumeRecovery(), discardRecovery: () => props.identity.discardRecovery() }));
    useEffect(() => {
    ...
    if (recoveryPending) {
        ...
        return _jsxs(OnboardingModal, { title: t(recoveryRequired ? 'onboardingRecoveryRequiredTitle' : 'onboardingConnectTitle'), closeLabel: t('onboardingClose'), onClose: dismiss, children: [identityAccess(identity.sessionStatus), _jsx("div", { className: css.actions, children: alternatives })] });
    }
