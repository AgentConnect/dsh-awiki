...
import { AWIKI_MODEL_PROXY_RPC_CHANNEL, AWIKI_MODEL_PROXY_RPC_ENDPOINTS, decodeCloseRechargeResult, decodeModelProxyCapability, decodeModelProxyStatus, decodeModelProxyUsage, decodeRechargeOrder, } from '@awiki/dsh-plugin/model-proxy-contract';
import { AWIKI_RECHARGE_DISABLED_ERROR, AWIKI_RECHARGE_ENABLED } from "./recharge-availability.js";
...
export class AwikiModelProxyController {
    ...
    disposed = false;
    generation = 0;
    constructor(connection, identity, rechargeEnabled = AWIKI_RECHARGE_ENABLED) {
    ...
    async probe() {
        if (this.disposed || this.view.capability === 'available')
            return;
        if (this.capabilityProbe !== undefined)
        ...
        try {
            ...
            if (this.capabilityProbe === pending)
    ...
    async load() {
        if (this.view.capability === 'unknown')
            await this.probe();
        if (this.view.capability !== 'available')
            return;
        if (!this.active())
            return;
    ...
    async createRecharge(amountCents) {
        if (!this.rechargeEnabled)
            throw new Error(AWIKI_RECHARGE_DISABLED_ERROR);
        if (!this.active())
    ...
    async call(endpoint, payload, lifetime = 'session') {
        ...
        if (!result.ok)
            throw new ModelProxyBrowserError(result.error.code);
        return result.value;
    }
    publish(next) {
