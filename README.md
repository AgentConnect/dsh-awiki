...
- Web identities use the same independent-device Join/revoke flow. The device page lets a current ready-admin edit public services, keeps protected system services read-only, and explicitly resumes a Core-owned pending update after a lost response. Web has no Recovery or Root Transfer; losing the first administrator permanently loses management capability. Unfinished registrations are rediscovered from Core and require an explicit choice plus fresh verification, even when new creation has been disabled. See [the method and restart contract](docs/dsh-device-join-design.md#45-web-身份方法与非设备服务更新).
