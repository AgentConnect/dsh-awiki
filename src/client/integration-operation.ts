...
export function integrationOperationId(): string {
...
function key(kind: IntegrationOperationKind): string {
  return `${STORAGE_PREFIX}${kind}`
}

export function durableIntegrationOperationId(
  ...
  try {
    const existing = JSON.parse(target.getItem(key(kind)) ?? 'null') as StoredOperation | null
  ...
  target.setItem(key(kind), JSON.stringify({ signature, id } satisfies StoredOperation))
  return id
}

export function clearIntegrationOperation(kind: IntegrationOperationKind): void {
