    ...
    <section className={css.page} aria-label="AWiki 设备管理">
      ...
      {snapshot?.canManage && (
        <>
          ...
          {!snapshot.rootTransferSupported && <div className={css.notice}><strong>管理权转移暂不可用</strong><span>{snapshot.methodCapabilities?.method === 'web' ? 'Web 身份不支持恢复或管理权转移；首个管理员丢失后无法恢复管理能力。' : '该功能目前只能在配备 Intel 芯片的 Mac 上通过系统身份验证使用。'}</span></div>}
