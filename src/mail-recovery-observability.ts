import type {
  ...
  AwikiMailRecoveryObservability,
} from './types.ts'

const MAIL_INGRESS_CLASSIFICATIONS = new Set<AwikiMailIngressClassification>([
  'reached',
  'not_reached',
  'unknown',
])
const AUTH_STATUS_CLASSES = new Set<AwikiMailAuthStatusClass>([
  'accepted',
  'rejected',
  'dependency_unavailable',
  'unknown',
])
const MAIL_CLOSED_CLASSIFICATIONS = new Set<AwikiMailClosedClassification>([
  'success',
...
export type AwikiMailRecoveryFailureFields = Pick<
  AwikiMailRecoveryObservability,
...
function optionalProperty(value: unknown, key: string):
  ...
  try {
    ...
    return { status: 'present', value: (value as Record<string, unknown>)[key] }
  } catch {
...
export function mailRecoveryFailureFields(error: unknown): AwikiMailRecoveryFailureFields | undefined {
  ...
  if (typeof ingress !== 'string' || !MAIL_INGRESS_CLASSIFICATIONS.has(ingress as AwikiMailIngressClassification)
    || typeof authStatus !== 'string' || !AUTH_STATUS_CLASSES.has(authStatus as AwikiMailAuthStatusClass)
    || typeof closed !== 'string' || !MAIL_CLOSED_CLASSIFICATIONS.has(closed as AwikiMailClosedClassification)
    || ingress === 'unknown'
    || authStatus === 'unknown'
    || closed === 'unknown'
    || closed === 'success'
  ...
  if (!CLOSED_FAILURE_TUPLES.has(tuple)) return undefined
  ...
  if (stableCode.status === 'unreadable'
    ...
    || (stableCode.status === 'present' && authStatus === 'accepted')) return undefined
  return {
    mail_ingress_classification: ingress as AwikiMailIngressClassification,
    auth_status_class: authStatus as AwikiMailAuthStatusClass,
    ...(stableCode.status === 'present'
      ? { auth_stable_machine_code: stableCode.value as string }
      : {}),
    retryable,
    mail_closed_classification: closed as AwikiMailClosedClassification,
  }
}

export function successfulMailRecoveryObservability(): AwikiMailRecoveryObservability {
  return {
...
export function failedMailRecoveryObservability(error: unknown): AwikiMailRecoveryObservability {
...
export function fencedMailRecoveryObservability(
  currentPrincipalMatchesRecovery: boolean,
  requestGenerationClassification: 'current' | 'replaced',
): AwikiMailRecoveryObservability {
  return {
