...
export type AwikiIntegrationTargetAvailability =
  | 'eligible'
  | 'group_not_found'
  | 'owner_not_active'
  | 'owner_mismatch'
...
export type AwikiIntegrationResult<Value> =
  ...
  | { readonly ok: false; readonly error: AwikiIntegrationFailure }
...
export interface AwikiDeviceManagementDevice {
  ...
  readonly provisioning?: { readonly phase: 'pending' | 'waiting' | 'failed' | 'complete'; readonly attempts: number; readonly requiresRejoin?: boolean }
...
export type AwikiRecoveryAction =
...
/** Explicitly download one attachment using the service ID from an inbox or sent row. */
...
export interface AwikiRuntimeConfig {
  ...
  readonly mailAttachmentDownloadMaxBytes: number
