...
export class AwikiSessionStore {
  ...
  private readonly recoveryOperationPath: string
  ...
  public async recoveryOperationId(): Promise<string | null> {
  ...
  public async setRecoveryOperationId(operationId: string | null): Promise<void> {
    if (operationId === null) {
    ...
    try {
      await writeFile(
        temporaryPath,
        `${RECOVERY_OPERATION_PREFIX}${operationId}\n`,
        { flag: 'wx', mode: 0o600 },
      )
