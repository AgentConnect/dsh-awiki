...
VERSIONS = {'@agent-network-protocol/dsh-anp-identity': '0.1.4-sg.20260922.2', '@awiki/dsh-plugin': '0.3.14-sg.20260923.1', '@awiki/dsh-model-proxy': '0.1.9-sg.20260923.1'}
...
def native_inputs(root):
    ...
    for platform in ['linux-x64', 'darwin-arm64', 'darwin-x64', 'linux-arm64', 'windows-x64']:
        ...
        for path in files:
            ...
            expected_version = ('0.2.9-sg.20260923.1' if name.startswith('@awiki/im-core-node')
    ...
    for directory in [consumer,consumer/'packages/dsh-model-proxy']:
        ...
        for group in ['dependencies','devDependencies','peerDependencies']:
            for name in list(manifest.get(group,{})):
                if name=='@awiki/im-core-node':manifest[group][name]='0.2.9-sg.20260923.1'
