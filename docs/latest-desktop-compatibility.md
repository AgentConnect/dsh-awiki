...
Release candidates: AWiki 0.3.11-rc.1 and Model Proxy 0.1.7-rc.1, intended for publication under the dedicated `dsh-desktop` npm tag for DSH 0.1.5-rc.1. Existing `latest` and `shanghai` tags are unchanged. Desktop 2.2.0-rc.1 already embeds the same implementation as local archives labeled 0.3.11/0.1.7; these standalone npm releases use explicit RC versions. Unit/build evidence does not substitute for actual desktop loading, persistence after reopening, or a real model request. Backend rollout and live account acceptance remain separate gates.
