...
describe('AwikiOverlay', () => {
  ...
  it('requires a server-validated invitation before OTP and forwards it through registration', async () => {
    ...
    b.fake.remote.inspectIdentityAccess = request => {
      ...
      return carried(success({ status: 'available', fullHandle: 'abc.awiki.info', inviteRequired: true,
        inviteStatus: request.inviteCode === 'accepted-invite' ? 'valid' : request.inviteCode ? 'invalid' : 'required' }))
    }
    fireEvent.click(screen.getByRole('button', { name: '打开 AWiki' }))
    fireEvent.change(await screen.findByLabelText('Handle'), { target: { value: 'abc' } })
    fireEvent.change(screen.getByLabelText('手机号'), { target: { value: '13800000000' } })
    fireEvent.click(screen.getByRole('button', { name: '获取验证码' }))
    const invitation = await screen.findByLabelText('邀请码')
    expect(b.fake.calls.filter(c => c.method === 'sendRegistrationOtp')).toHaveLength(0)
    expect(screen.getByRole('button', { name: '获取验证码' })).toHaveProperty('disabled', true)
    fireEvent.change(invitation, { target: { value: 'wrong' } })
    fireEvent.click(screen.getByRole('button', { name: '获取验证码' }))
    expect(await screen.findByText('邀请码无效或不适用于当前账号，请检查后重试。')).toBeTruthy()
    expect(b.fake.calls.filter(c => c.method === 'sendRegistrationOtp')).toHaveLength(0)
    fireEvent.change(invitation, { target: { value: ' accepted-invite ' } })
    fireEvent.click(screen.getByRole('button', { name: '获取验证码' }))
    fireEvent.change(await screen.findByLabelText('注册验证码'), { target: { value: '123456' } })
    expect(b.fake.calls.find(c => c.method === 'sendRegistrationOtp')?.request).toEqual({ handle: 'abc', phone: '13800000000', inviteCode: 'accepted-invite' })
    fireEvent.click(screen.getByRole('button', { name: '继续' }))
    fireEvent.click(screen.getByRole('button', { name: '确认并继续' }))
    await waitFor(() => expect(b.fake.calls.find(c => c.method === 'registerIdentity')?.request).toEqual({ handle: 'abc', phone: '13800000000', otp: '123456', inviteCode: 'accepted-invite' }))
    expect(JSON.stringify(window.localStorage)).not.toContain('accepted-invite')
    expect(JSON.stringify(window.sessionStorage)).not.toContain('accepted-invite')
  })

  it('clears the invitation when changing Handle and lets an existing three-character account continue', async () => {
    const b = renderOverlay({ registered: false })
    b.fake.remote.inspectIdentityAccess = request => carried(success(request.handle === 'new'
      ? { status: 'available', fullHandle: 'new.awiki.info', inviteRequired: true, inviteStatus: 'required' }
      : { status: 'existing', fullHandle: 'old.awiki.info', inviteRequired: false, inviteStatus: 'not_required' }))
    fireEvent.click(screen.getByRole('button', { name: '打开 AWiki' }))
    fireEvent.change(await screen.findByLabelText('Handle'), { target: { value: 'new' } })
    fireEvent.change(screen.getByLabelText('手机号'), { target: { value: '13800000000' } })
    fireEvent.click(screen.getByRole('button', { name: '获取验证码' }))
    fireEvent.change(await screen.findByLabelText('邀请码'), { target: { value: 'private-invite' } })
    fireEvent.change(screen.getByLabelText('Handle'), { target: { value: 'old' } })
    expect(screen.queryByLabelText('邀请码')).toBeNull()
    fireEvent.click(screen.getByRole('button', { name: '获取验证码' }))
    await screen.findByLabelText('注册验证码')
    expect(b.fake.calls.find(c => c.method === 'sendRegistrationOtp')?.request).toEqual({ handle: 'old', phone: '13800000000' })
  })
  ...
  it('retains selected Files across drawer remounts without duplicate removal ids', async () => {
    ...
    fireEvent.change(screen.getByLabelText('添加附件'), { target: { files: [first] } })
    ...
    fireEvent.change(screen.getByLabelText('添加附件'), { target: { files: [second] } })
    fireEvent.click(screen.getByRole('button', { name: '移除附件 second.txt' }))
    expect(screen.getByText('first.txt')).toBeTruthy()
    expect(screen.queryByText('second.txt')).toBeNull()
    expect(b.fake.calls.filter(call => call.method === 'sendMail')).toHaveLength(0)
    expect(window.localStorage.getItem('mail:attachments')).toBeNull()
  })

  it('does not send an old draft after the drawer closes while a File is being read', async () => {
    const b = renderOverlay()
    await openMailCompose(); fillMailDraft()
    const pending = deferred<ArrayBuffer>()
    const file = new File(['abc'], 'delayed.txt', { type: 'text/plain' })
    ...
    fireEvent.click(screen.getByRole('button', { name: '确认发送' }))
    fireEvent.click(screen.getByRole('button', { name: '关闭 AWiki' }))
    pending.resolve(new Uint8Array([97, 98, 99]).buffer)
    await waitFor(() => expect(b.fake.calls.filter(call => call.method === 'sendMail')).toHaveLength(0))
    await openMailCompose()
    expect(screen.getByText('delayed.txt')).toBeTruthy()
  })
  ...
  it.each(['alice', 'abc', 'abcd'])('fails closed before OTP when account inspection fails: %s', async (handle) => {
    const b = renderOverlay({ registered: false })
    b.fake.remote.inspectIdentityAccess = (request) => {
      b.fake.calls.push({ method: 'inspectIdentityAccess', request })
    ...
    expect(b.fake.calls.filter(call => call.method === 'sendRecoveryOtp')).toHaveLength(0)
    expect(screen.getByLabelText('Handle')).toHaveProperty('value', handle)
    expect(screen.getByLabelText('手机号')).toHaveProperty('value', '13800000000')
  })

  it.each(['abc', 'abcd'])('shows the server short-Handle invitation notice after registration submission: %s', async (handle) => {
    const b = renderOverlay({ registered: false })
    b.fake.remote.registerIdentity = (request) => {
      b.fake.calls.push({ method: 'registerIdentity', request })
      return carried({
        ok: false,
        error: { code: 'short-handle-invite-required', message: 'private service invitation detail' },
      })
    }
    fireEvent.click(screen.getByRole('button', { name: '打开 AWiki' }))
    fireEvent.change(await screen.findByLabelText('Handle'), { target: { value: handle } })
    ...
    expect(await screen.findByText('此 Handle 注册需要邀请码，请输入后继续。')).toBeTruthy()
    ...
    expect(b.fake.calls.filter(call => call.method === 'registerIdentity')).toEqual([{
      method: 'registerIdentity', request: { handle, phone: '13800000000', otp: '123456' },
    }])
    expect(screen.queryByLabelText('注册验证码')).toBeNull()

    const message = '此 Handle 注册需要邀请码，请输入后继续。'
    fireEvent.click(screen.getByRole('button', { name: '关闭 AWiki' }))
  ...
  it('does not infer the short-Handle invitation notice from registration error text', async () => {
    const b = renderOverlay({ registered: false })
    b.fake.remote.registerIdentity = (request) => {
      b.fake.calls.push({ method: 'registerIdentity', request })
      return carried({
        ...
        error: {
          ...
          message: '此 Handle 注册需要邀请码，请输入后继续。',
        },
      })
    }
    fireEvent.click(screen.getByRole('button', { name: '打开 AWiki' }))
    ...
    expect(screen.queryByText('此 Handle 注册需要邀请码，请输入后继续。')).toBeNull()
  })

  it('allows an existing short Handle to continue through registration OTP for Join or recovery', async () => {
    const b = renderOverlay({
      registered: false,
      identityAccessInspection: { status: 'existing', fullHandle: 'abc.awiki.info' },
    })
    fireEvent.click(screen.getByRole('button', { name: '打开 AWiki' }))
    fireEvent.change(await screen.findByLabelText('Handle'), { target: { value: 'abc' } })
    fireEvent.change(screen.getByLabelText('手机号'), { target: { value: '13800000000' } })
  ...
  it('renders a Host short-Handle policy race as the invitation notice without leaking forbidden copy', async () => {
    const b = renderOverlay({
      registered: false,
      identityAccessInspection: { status: 'existing', fullHandle: 'abcd.awiki.info' },
    })
    b.fake.remote.sendRegistrationOtp = (request) => {
      b.fake.calls.push({ method: 'sendRegistrationOtp', request })
      return carried({
        ok: false,
        error: { code: 'short-handle-invite-required', message: 'private service invitation detail' },
      })
    ...
    expect(await screen.findByText('此 Handle 注册需要邀请码，请输入后继续。')).toBeTruthy()
    expect(document.body.textContent).not.toContain('当前 AWiki 服务未向该手机号开放注册')
    expect(document.body.textContent).not.toContain('private service invitation detail')
    expect(screen.queryByLabelText('注册验证码')).toBeNull()
  })
  ...
  it('turns a revoked device into a device rejoin flow without offering Handle recovery', async () => {
    ...
    expect(screen.queryByRole('button', { name: '发送恢复验证码并替换 DID' })).toBeNull()
  ...
  it('does not offer Recovery to an active session when capability and identity-access discovery are unavailable', async () => {
    const b = renderOverlay({
      config: { pollIntervalMs: 1_000, attachmentMaxBytes: 1_024, handleRecoveryPhoneEnabled: false },
    })
    b.fake.remote.getIdentityAccessState = () => carried({
      ok: false,
      error: { code: 'network', message: 'private active identity status detail' },
    })

    fireEvent.click(screen.getByRole('button', { name: '打开 AWiki' }))

    expect(await screen.findByRole('button', { name: '重新检查身份状态' })).toBeTruthy()
    await waitFor(() => {
      expect(b.controller.getSnapshot()).toMatchObject({
        sessionStatus: 'active',
        identity,
    ...
    expect(document.body.textContent).not.toContain('private active identity status detail')
  })

  it('keeps Recovery for a recovery-required session when capability discovery is unavailable', async () => {
    const b = renderOverlay({
      config: { pollIntervalMs: 1_000, attachmentMaxBytes: 1_024, handleRecoveryPhoneEnabled: false },
    })
    fireEvent.click(screen.getByRole('button', { name: '打开 AWiki' }))
    ...
    b.fake.remote.getIdentityAccessState = () => carried({
      ok: false,
      error: { code: 'network', message: 'private identity status detail' },
    })
    b.fake.remote.listConversations = request => {
      b.fake.calls.push({ method: 'listConversations', request })
      return carried({
        ok: false,
        error: { code: 'identity-recovery-required', message: 'private revoked credential detail' } as never,
      })
    }

    fireEvent.click(screen.getByRole('button', { name: '刷新 AWiki' }))

    expect(await screen.findByRole('button', { name: '恢复身份' })).toBeTruthy()
    expect(screen.getByRole('button', { name: '重新检查身份状态' })).toBeTruthy()
    expect(b.fake.calls.filter(call => call.method === 'sendRecoveryOtp')).toHaveLength(0)
    expect(document.body.textContent).not.toMatch(/private identity status detail|private revoked credential detail/u)
    fireEvent.click(screen.getByRole('button', { name: '恢复身份' }))
    expect(await screen.findByRole('heading', { name: '需要重新恢复身份' })).toBeTruthy()
    expect(screen.getByRole('button', { name: '发送恢复验证码并替换 DID' })).toBeTruthy()
    expect(b.fake.calls.filter(call => call.method === 'sendRecoveryOtp')).toHaveLength(0)
    expect(document.body.textContent).not.toMatch(/private identity status detail|private revoked credential detail/u)
  })

  it('clears a recovery-required entry when the remounted session becomes active with discovery failure', async () => {
    const b = renderOverlay({
      config: { pollIntervalMs: 1_000, attachmentMaxBytes: 1_024, handleRecoveryPhoneEnabled: false },
    })
    fireEvent.click(screen.getByRole('button', { name: '打开 AWiki' }))
    ...
    b.fake.remote.getIdentityAccessState = () => carried({
      ok: false,
      error: { code: 'network', message: 'private remount discovery sentinel' },
    })
    b.fake.remote.listConversations = () => carried({
      ok: false,
      error: { code: 'identity-recovery-required', message: 'private revoked sentinel' } as never,
    })
    fireEvent.click(screen.getByRole('button', { name: '刷新 AWiki' }))
    fireEvent.click(await screen.findByRole('button', { name: '恢复身份' }))
    expect(await screen.findByRole('heading', { name: '需要重新恢复身份' })).toBeTruthy()

    b.fake.remote.listConversations = request => {
      b.fake.calls.push({ method: 'listConversations', request })
      return carried(success({ items: [direct], hasMore: false }))
    }
    fireEvent.click(screen.getByRole('button', { name: '刷新 AWiki' }))
    await waitFor(() => { expect(b.controller.getSnapshot().sessionStatus).toBe('active') })
    fireEvent.click(screen.getByRole('button', { name: '关闭 AWiki' }))
    fireEvent.click(screen.getByRole('button', { name: '打开 AWiki' }))
    ...
    expect(document.body.textContent).not.toMatch(/private remount discovery sentinel|private revoked sentinel/u)
  })
  ...
  it('keeps Device Join active until a second explicit Recovery confirmation sends one OTP', async () => {
    const b = renderOverlay({
      registered: false,
    ...
    fireEvent.click(screen.getByRole('button', { name: '取消，返回普通加入' }))
    expect(await screen.findByRole('button', { name: '加入新设备（推荐）' })).toBeTruthy()
    expect(b.fake.calls.filter(call => call.method === 'cancelDeviceJoin')).toHaveLength(0)
    expect(b.fake.calls.filter(call => call.method === 'sendRecoveryOtp')).toHaveLength(0)

    fireEvent.click(screen.getByRole('button', { name: '恢复 Handle（会替换 DID）' }))
    const confirm = await screen.findByRole('button', { name: '发送恢复验证码并替换 DID' })
    fireEvent.click(confirm)
    fireEvent.click(confirm)
    expect(await screen.findByRole('heading', { name: '验证身份归属' })).toBeTruthy()
    expect(b.fake.calls.filter(call => call.method === 'cancelDeviceJoin')).toHaveLength(1)
  ...
  it('projects a Join carrier failure before it reaches controller state or the DOM', async () => {
    const sentinel = 'private Join carrier sentinel: token=secret'
    const b = renderOverlay({
      registered: false,
      config: { pollIntervalMs: 1_000, attachmentMaxBytes: 1_024, handleRecoveryPhoneEnabled: true },
      registrationOutcome: {
    ...
    expect((await screen.findByRole('alert')).textContent).toBe('暂时无法连接 AWiki Host，请稍后重试。')
    expect(document.body.textContent).not.toContain(sentinel)
    expect(JSON.stringify(b.controller.getSnapshot())).not.toContain(sentinel)
  })
  ...
  it('restores a failed attachment draft after showing only its safe metadata in the optimistic bubble', async () => {
    ...
    fail()
    expect(await screen.findByText('无法连接 AWiki 服务，请检查网络后重试。')).toBeTruthy()
  ...
  it('keeps the file card and reports an image preview failure', async () => {
    ...
    expect(await screen.findByText('AWiki 暂时无法完成此操作，请稍后重试。')).toBeTruthy()
  ...
  it('keeps the closed launcher mounted and offers retry after a refresh failure', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '刷新 AWiki' }))
    expect(await screen.findByText('暂时无法连接 AWiki Host，请稍后重试。')).toBeTruthy()
