...
it('keeps Web Join on the ordinary member approval path', async () => {
  const fixture = rustFixture()
  fixture.client.getDefaultIdentity = async () => ({ ...NODE_IDENTITY, did: 'did:web:example.test:alice' })
  fixture.client.identityMethodCapabilities = async () => ({ method: 'web', handleRecovery: false, rootImport: false, rootTransfer: false, servicesUpdate: true })
  fixture.client.prepareDeviceJoinApproval = async () => ({ approvalHandle: 'web-approval', joinSessionId: 'web-join', sas: '123456', expiresAt: '2030-01-01T00:00:00Z' })
  const ordinary = vi.fn(async () => ({ joinSessionId: 'web-join', localPhase: 'authorized', remotePhase: 'authorized', expiresAt: '2030-01-01T00:00:00Z' }))
  const management = vi.fn()
  Object.assign(fixture.client, { confirmDeviceJoinApproval: ordinary, confirmDeviceJoinWithManagement: management })
  await fixture.adapter.prepareDeviceJoinApproval('web-join')
  await fixture.adapter.confirmDeviceJoinApproval('web-approval')
  expect(ordinary).toHaveBeenCalledExactlyOnceWith({ approvalHandle: 'web-approval', userPresenceConfirmed: true })
