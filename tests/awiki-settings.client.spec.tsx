...
function mount(snapshot: AwikiTenantScopeSnapshot = ready(), overrides: Record<string, unknown> = {}) {
  const actions = {
    ...
    loadAwiki: vi.fn(async () => ({ ok: true, value: undefined })),
    refreshDeviceManagement: vi.fn(async () => ({
      ok: true,
      value: {
        canManage: true,
    ...
    startDeviceJoinVerification: vi.fn(async () => ({ ok: false, error: 'unexpected verification' })),
    approveDeviceJoin: vi.fn(async () => ({ ok: false, error: 'unexpected approval' })),
    rejectDeviceJoin: vi.fn(async () => ({ ok: false, error: 'unexpected rejection' })),
    revokeDevice: vi.fn(async () => ({ ok: false, error: 'unexpected revoke' })),
    prepareRootTransfer: vi.fn(async () => ({ ok: false, error: 'unexpected transfer' })),
    confirmRootTransfer: vi.fn(async () => ({ ok: false, error: 'unexpected transfer confirmation' })),
    loadIntegration: vi.fn(async () => ({ ok: false, error: 'unavailable' })),
    saveIntegration: vi.fn(async () => ({ ok: false, error: 'unavailable' })),
    rotateIntegrationId: vi.fn(async () => ({ ok: false, error: 'unavailable' })),
    closeIntegration: vi.fn(async () => ({ ok: false, error: 'unavailable' })),
    reopenIntegration: vi.fn(async () => ({ ok: false, error: 'unavailable' })),
    listOwnedGroups: vi.fn(async () => ({ ok: true, value: [] })),
    ...overrides,
  }
...
describe('AWiki tenant-aware settings section', () => {
  it('combines tenant, device, local-data, and integration settings', async () => {
    ...
    fireEvent.click(screen.getByRole('tab', { name: '临时消息集成' }))
