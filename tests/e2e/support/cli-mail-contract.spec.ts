...
const summary = { id: 'mail-1', subject: expected.subject, to: [expected.to], from: [expected.from] }
function fixture() {
...
it.each([
  ...
  const { peer, run } = fixture()
  run.mockResolvedValueOnce({ data: { messages: [summary] } }).mockResolvedValueOnce({ data: detail })
