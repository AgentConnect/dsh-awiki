...
test('[DSH-WEB-MAIL-RECOVERY-001] Clear Local Data Recovery restores server inbox and outbound history', async ({ browser, harness }) => {
  ...
  const mailHarness = await startHarnessInstance({ isolated: true, profileSource: harness.dshHome })
  ...
  try {
    ...
    await sendVisibleMail(page, config.mailEchoRecipient, baselineSubject, `echo ${baselineSubject}`)
