...
test('[DSH-WEB-SMOKE-001] real Harness loads the AWiki Web launcher and identity entry', async ({ page, harness }) => {
  ...
  const legacy = settings.locator('article').filter({ hasText: 'rwiki.cn' })
  ...
  await expect(legacy.getByText(/^(?:当前|Current)$/u)).toBeVisible()
  await china.getByRole('button', { name: /^(?:切换|Switch)$/u }).click()
