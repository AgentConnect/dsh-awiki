...
test('[DSH-WEB-RECOVERY-001] Fresh Root Recovery replaces DID, fences old CLI, and survives restart', async ({ browser, harness }) => {
  ...
  await sourcePage.getByRole('dialog', { name: '确认创建 AWiki 身份' }).getByRole('button', { name: '确认并继续' }).click()
  ...
  try {
    ...
    const operationId = await page.locator('details code').first().innerText()
    await page.getByRole('button', { name: '返回身份入口' }).click()
    await page.getByLabel('Handle').fill(localHandle)
    await page.getByLabel('手机号').fill(config.phone)
    await page.getByRole('button', { name: '获取验证码' }).click()
    await expect(page.getByRole('heading', { name: '验证身份归属' })).toBeVisible()
    await expect(page.locator('details code').first()).toHaveText(operationId)
    // Fresh browser state discovers the same Core operation only after the user chooses its Handle.
    await context.close()
