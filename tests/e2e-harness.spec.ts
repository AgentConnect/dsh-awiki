...
describe('DSH Web E2E Harness contract', () => {
  ...
  it('reads the Identity SDK resolved by its plugin in a pnpm profile without a root SDK link', async () => {
    ...
    await writeFile(join(sdk, 'package.json'), JSON.stringify({ name: '@agent-network-protocol/anp-identity', version: '0.2.2', exports: { '.': './index.js' } }))
    ...
    await expect(readProfileIdentityManifest(root)).resolves.toMatchObject({ name: '@agent-network-protocol/anp-identity', version: '0.2.2' })
  ...
  it('pins selected native platforms as well as wrappers without selecting an unrelated SDK', () => {
    ...
    expect(nativeProfileOverrides({ identity: { wrapper: '/identity.tgz', platform: '/identity-linux.tgz', target: 'linux-x64-gnu' } }))
      .toEqual({ '@awiki/im-core-node': '0.2.8', '@agent-network-protocol/anp-identity': 'file:/identity.tgz', '@agent-network-protocol/anp-identity-linux-x64-gnu': 'file:/identity-linux.tgz' })
  })
