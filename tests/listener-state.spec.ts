...
describe('AWiki Agent listener state', () => {
  it('atomically persists only peer routes, message watermarks, and DSH session ids', async () => {
    const root = await temporaryRoot()
    const store = new AwikiListenerStateStore(root)
    await expect(store.load()).resolves.toEqual({ version: 1, conversations: {} })
    ...
    await expect(new AwikiListenerStateStore(root).load()).resolves.toEqual({
      version: 1,
      conversations: {
        'conversation-1': {
          peerDid: 'did:awiki:bob',
  ...
  it('rejects malformed state and a symlinked Host directory', async () => {
    ...
    await expect(new AwikiListenerStateStore(root).load()).rejects.toThrow('listener state')

    if (process.platform !== 'win32') {
      const linkedRoot = await temporaryRoot()
      const target = await temporaryRoot()
      await symlink(target, join(linkedRoot, '.host'))
      await expect(new AwikiListenerStateStore(linkedRoot).load()).rejects.toThrow('local session directory')
    }
  })
})
